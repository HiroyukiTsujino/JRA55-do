...
program calibration_surface_temperature
  ...
  character(256) :: file_calib_all
  character(256) :: file_calib_ice

  real(8),allocatable :: sat_all(:,:,:)
  ...
  open(lun,file='namelist.calibsat',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.calibsat'
    stop
  end if
  ...
  if ( ios /= 0 ) then
    write(6,*) ' Read error : namelist.calibsat', ios
    stop
  end if
  ...
  allocate(sat_all(1:imf,1:jmf,1:num_month))
  ...
  lreclen = 4 * imf * jmf
  call open_file_direct(mtin1, file_calib_all, lreclen, convert_mode='little_endian')
  ...
  do m = 1, num_month
    ...
    sat_all(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
  end do
  call close_file(mtin1)

  lreclen = 4 * imf * jmf
  call open_file_direct(mtin1, file_calib_ice, lreclen, convert_mode='little_endian')
    ...
    do j = 1, jmf
      do i = 1, imf
        if (icef(i,j) > 0.0d0) then
          ...
          satc(i,j) = satc(i,j) + (sat_all(i,j,monb) * bfm + sat_all(i,j,mona) * afm)
        end if
      end do
    end do
