! -*-F90-*-
!------------------ calib_wind_offset_interannual.F90 ---------------------
!  Information:
...
program calibration_surface_wind
  ...
  character(256) :: file_calib_rot_a, file_calib_rot_b

  real(8),allocatable :: wind_mag_a(:,:,:), wind_mag_b(:,:,:)
  real(8),allocatable :: wind_rot_a(:,:,:), wind_rot_b(:,:,:)
  ...
  real(8),parameter :: wind_speed_min = 0.2d0 ! 0.3 [m/s] floor for wind speed
  ...
  file_calib_rot_a = 'no_file'
  file_calib_rot_b = 'no_file'

  open(lun,file='namelist.calibwind_offset',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.calibwind_offset'
    stop
  end if
  ...
  rewind(lun)
  read(lun, nml=nml_calibwind_offset, iostat=istat)
  ios = ios + istat
  ...
  allocate(wind_rot_a(1:imf,1:jmf,1:num_month))
  allocate(wind_rot_b(1:imf,1:jmf,1:num_month))
  ...
  lreclen = 4 * imf * jmf
  call open_file_direct(mtin1, file_calib_rot_a, lreclen, &
       & convert_mode='little_endian',action='read')
  ...
  do m = 1, num_month
    ...
    wind_rot_a(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
  end do
  call close_file(mtin1)
  ...
  if (l_calib_transition) then
    ...
    write(6,*) ' Factor (b) : ', trim(file_calib_rot_b)
    write(6,*) ' Factor (a) : ', trim(file_calib_mag_a)
    write(6,*) ' Factor (a) : ', trim(file_calib_rot_a)
    ...
    lreclen = 4 * imf * jmf
    call open_file_direct(mtin1, file_calib_rot_b, lreclen, &
         & convert_mode='little_endian',action='read')
    ...
    do m = 1, num_month
      ...
      wind_rot_b(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
    end do
    call close_file(mtin1)
...
!!!    write(6,*) ' reading from ', trim(fin_vs)
    ...
    do j = 1, jmf
      do i = 1, imf
        ...
        rot_a = wind_rot_a(i,j,monb) * bfm + wind_rot_a(i,j,mona) * afm

        if (l_calib_transition) then
          mag_b = wind_mag_b(i,j,monb) * bfm + wind_mag_b(i,j,mona) * afm
          rot_b = wind_rot_b(i,j,monb) * bfm + wind_rot_b(i,j,mona) * afm
          mag_this = act * mag_a + bct * mag_b
          rot_this = act * rot_a + bct * rot_b
        else
          mag_this = mag_a
          rot_this = rot_a
        end if

        hl2 = rot_this * radian_r ! degree to radian

        wind_speed = sqrt(usf(i,j)**2 + vsf(i,j)**2)

        if (wind_speed < wind_speed_min) then
          write(6,*) ' wind speed error (wind_speed) ', wind_speed, i,j
          stop
        end if

        if (abs(mag_this) > 0.0d0) then

          hl1 = max(wind_speed - wind_speed_cut, 0.0d0) / abs(mag_this)

          if (hl1 < 0.0d0) then
            write(6,*) ' wind speed error (hl1) '
            stop
          end if
        
          wind_speed_adj = wind_speed + mag_this * tanh(hl1)

          if (wind_speed_adj < wind_speed_min) then
            write(6,*) ' wind speed error (wind_speed_adj) ', wind_speed_adj,wind_speed,mag_this
            stop
          end if

          hl3 = wind_speed_adj / wind_speed
        ...
        usc(i,j) = hl3 * (usf(i,j) * cos(hl2) - vsf(i,j) * sin(hl2))
        vsc(i,j) = hl3 * (usf(i,j) * sin(hl2) + vsf(i,j) * cos(hl2))
