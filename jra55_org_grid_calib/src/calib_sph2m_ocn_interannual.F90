! -*-F90-*-
!---------------- calib_sph2m_ocn_interannual.F90 ------------------
!  Information:
...
program calibration_surface_temperature
  ...
  open(lun,file='namelist.calibsph_ocn',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.calibsph_ocn'
    stop
  end if
  ...
  rewind(lun)
  read(lun, nml=nml_calibsph_ocn, iostat=istat)
  ios = ios + istat

  if ( ios /= 0 ) then
    write(6,*) ' Read error : namelist.calibsph_ocn', ios
    stop
  end if
  ...
  allocate(sph_ocn(1:imf,1:jmf,1:num_month))
  ...
  do m = 1, num_month
    ...
    sph_ocn(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
  end do
  call close_file(mtin1)
  ...
  do m = 1, num_month
    do j = 1, jmf
      do i = 1, imf
        if ((msk(i,j) == 1.0d0) .and. (sph_ocn(i,j,m) == real(undef_calib,8))) then
    ...
    do j = 1, jmf
      do i = 1, imf
        if ((msk(i,j) == 1.0d0) .and. (icef(i,j) == 0.0d0)) then
          sphc(i,j) = sphc(i,j) * (sph_ocn(i,j,monb) * bfm + sph_ocn(i,j,mona) * afm)
