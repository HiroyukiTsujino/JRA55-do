! -*-F90-*-
!---------------- calib_tmp2m_ice_interannual.F90 ------------------
!  Information:
...
program calibration_surface_temperature
  ...
  namelist /nml_calibsat_ice/ calc_start, calc_end, &
       &  l_leap_valid, &
       &  l_calib_monthly, &
       &  file_calib_ice, &
       &  undef_calib, &
  ...
  open(lun,file='namelist.calibsat_ice',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.calibsat_ice'
    stop
  end if
  ...
  rewind(lun)
  read(lun, nml=nml_calibsat_ice, iostat=istat)
  ios = ios + istat

  if ( ios /= 0 ) then
    write(6,*) ' Read error : namelist.calibsat_ice', ios
    stop
  end if
  ...
  do m = 1, num_month
    do j = 1, jmf
      do i = 1, imf
        if ((msk(i,j) == 1.0d0) .and. (sat_ice(i,j,m) == real(undef_calib,8))) then
