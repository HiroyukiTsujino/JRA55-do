! -*-F90-*-
!-------------- calib_tmp2m_ocn_ice_interannual.F90 ----------------
!  Information:
...
program calibration_surface_temperature
  ...
  character(256) :: file_calib_ocn_a
  character(256) :: file_calib_ice_a
  character(256) :: file_calib_ocn_b
  character(256) :: file_calib_ice_b

  real(8),allocatable :: sat_ocn_a(:,:,:)
  real(8),allocatable :: sat_ice_a(:,:,:)

  real(8),allocatable :: sat_ocn_b(:,:,:)
  real(8),allocatable :: sat_ice_b(:,:,:)
  ...
  file_calib_ocn_a='no_file'
  file_calib_ocn_b='no_file'
  file_calib_ice_a='no_file'
  file_calib_ice_b='no_file'

  open(lun,file='namelist.calibsat_ocn_ice',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.calibsat_ocn_ice'
    stop
  end if
  ...
  if ( ios /= 0 ) then
    write(6,*) ' Read error : namelist.calibsat_ocn_ice', ios
    stop
  end if
  ...
  allocate(sat_ocn_a(1:imf,1:jmf,1:num_month))
  allocate(sat_ice_a(1:imf,1:jmf,1:num_month))
  allocate(sat_ocn_b(1:imf,1:jmf,1:num_month))
  allocate(sat_ice_b(1:imf,1:jmf,1:num_month))
  ...
  lreclen = 4 * imf * jmf
  call open_file_direct(mtin1, file_calib_ocn_a, lreclen, convert_mode='little_endian')
  ...
  do m = 1, num_month
    ...
    sat_ocn_a(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
  end do
  call close_file(mtin1)

  lreclen = 4 * imf * jmf
  call open_file_direct(mtin1, file_calib_ice_a, lreclen, convert_mode='little_endian')
  ...
  do m = 1, num_month
    ...
    sat_ice_a(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
  end do
  call close_file(mtin1)

  if (l_calib_transition) then

    write(6,*) ' make transition of calibration factor (ocn)'
    write(6,*) ' Factor (b) : ', trim(file_calib_ocn_b)
    write(6,*) ' Factor (a) : ', trim(file_calib_ocn_a)

    lreclen = 4 * imf * jmf
    call open_file_direct(mtin1, file_calib_ocn_b, lreclen, convert_mode='little_endian')
    ...
    do m = 1, num_month
      ...
      sat_ocn_b(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
    end do
    call close_file(mtin1)

    write(6,*) ' make transition of calibration factor (ice)'
    write(6,*) ' Factor (b) : ', trim(file_calib_ice_b)
    write(6,*) ' Factor (a) : ', trim(file_calib_ice_a)

    lreclen = 4 * imf * jmf
    call open_file_direct(mtin1, file_calib_ice_b, lreclen, convert_mode='little_endian')
    ...
    do m = 1, num_month
      ...
      sat_ice_b(1:imf,1:jmf,m) = real(dat4f(1:imf,1:jmf),8)
    end do
    call close_file(mtin1)
  ...
  LOOP_SURF_FORCE: do while ( .true. )
    ...
    do j = 1, jmf
      do i = 1, imf
        !if (msk(i,j) == 1.0d0) then ! ocean grid
        ...
        !  write(6,'(i8,2f5.1,3f12.5)') i,msk(i,j),icef(i,j),sphf(i,j),qsat,rlhf(i,j)
        !end if
        !end if
      end do
    end do
    ...
    do j = 1, jmf
      do i = 1, imf
        if (icef(i,j) > 0.0d0) then
          calib_a = (sat_ice_a(i,j,monb) * bfm + sat_ice_a(i,j,mona) * afm)
          if (l_calib_transition) then
            calib_b = (sat_ice_b(i,j,monb) * bfm + sat_ice_b(i,j,mona) * afm)
            calib = act * calib_a + bct * calib_b
          else
            calib = calib_a
          end if
        else
          calib_a = (sat_ocn_a(i,j,monb) * bfm + sat_ocn_a(i,j,mona) * afm)
          if (l_calib_transition) then
            calib_b = (sat_ocn_b(i,j,monb) * bfm + sat_ocn_b(i,j,mona) * afm)
            calib = act * calib_a + bct * calib_b
          else
            calib = calib_a
          end if
        end if
        satc(i,j) = satc(i,j) + calib
      end do
    end do
