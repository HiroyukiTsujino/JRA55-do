...
SUBROUTINE GET_SCAT_DAILY_V4(FILENAME,MINGMT,WINDSPD,WINDDIR,SCATFLAG,RADRAIN,IERR,nx,ny)
  ...
  real(4),intent(out)    :: windspd(nx,ny,2), winddir(nx,ny,2)
  real(4),intent(out)    :: mingmt (nx,ny,2), radrain(nx,ny,2)
  integer(4),intent(out) :: scatflag(nx,ny,2)

  !real(4)    :: uu(nx,ny,2),vv(nx,ny,2)

  integer(4) :: ival(nx,ny)
  integer(4) :: iasc
  integer(4) :: lreclen
  logical(4) :: lexist

  character(len=1) :: data_array(nx,ny,4,2)
  ...
  OPEN(11,FILE=filename,STATUS='old',ACTION='read',&
       & RECL=lreclen,ACCESS='direct',FORM='unformatted')
  write(6,*) 'file open ', trim(filename), ' successful '
  READ(11,rec=1) data_array
  CLOSE(11)
...
END SUBROUTINE GET_SCAT_DAILY_V4

!***************************************************************************************
SUBROUTINE get_rain(data_array, scatflag,radrain)

  character(1),intent(in) :: data_array(1440,720,4,2)
  integer(4),intent(out)   :: scatflag(1440,720,2)
  real(4),intent(out)      :: radrain (1440,720,2)
  ...
  do iasc = 1,2
	...
	scatflag(ilon,ilat,iasc) = IBITS(ival,0,1)  !start at zero, extract 1 bit 
                                                    !independant scatterometer rain flag 
