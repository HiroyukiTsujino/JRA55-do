...
program reduced_grid_to_latlon_grid
  ...
  namelist /nml_rg2latlon_nointpol/ file_in, file_out, imut, jmut, dlon, grid_name

  !---------------------------------------------

  open(lun,file='namelist.rg2latlon_nointpol')
  read(lun,nml=nml_rg2latlon_nointpol)
  close(lun)
      ...
      do i = 1, imut
        do ii = 1, num_xgrid(j)
          if (abs(lon_org(ii) - lon_new(i)) < 1.0d-4) then
            ...
            if (weight <= 0.5d0) then
              data_new(i) = data_org(ii)
            else
              data_new(i) = data_org(ii+1)
            end if
            exit
          end if
        end do
