! -*-F90-*-
program reduced_grid_to_latlon_grid_mask
  ...
  integer(4),allocatable :: idata_latlon(:,:)
  ...
  character(256) :: file_out, file_topo
  ...
  namelist /nml_rg2latlon_mask/ file_in, file_out, file_topo, &
       & imut, jmut, dlon, grid_name

  !---------------------------------------------

  open(lun,file='namelist.rg2latlon_mask')
  read(lun,nml=nml_rg2latlon_mask)
  close(lun)
  ...
  allocate(idata_latlon(1:imut,1:jmut))
  ...
  read(lun,rec=1) data_rg ! 1: land, 0:water
  close(lun)
  ...
  do j = 1, jmut
    ...
    if (num_xgrid(j) == imut) then
      data_latlon(1:imut,jmut-j+1) = 1.0d0 - real(data_rg(ibgn:iend),4) ! 1: land, 0:water
    else
      ...
      do i = 1, imut
        do ii = 1, num_xgrid(j)
          if (abs(lon_org(ii) - lon_new(i)) < 1.0d-4) then
            ...
            if (data_new(i) > (0.5d0 + 1.0d-4)) then ! changed from >= for v0.3
              data_new(i) = 0.0d0
            else
              data_new(i) = 1.0d0
            end if
  ...
  idata_latlon(1:imut,1:jmut) = nint(data_latlon(1:imut,1:jmut))

  open(lun,file=file_topo,form='unformatted',access='sequential')
  write(lun) idata_latlon, idata_latlon
  close(lun)
...
end program reduced_grid_to_latlon_grid_mask
