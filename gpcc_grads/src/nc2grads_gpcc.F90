...
program read_netcdf_output_grads
  ...
  integer(4), parameter :: start_year = 1901
  ...
  real(4),parameter :: undef_gpcc = -9.96921e36
  ...
  open (10,file='namelist.gpcc')
  read (10,nml=nml_gpcc)
  close(10)
  ...
  do nyear = ibyr, ieyr
    do m = 1, 12
      ...
      do j = 1, jmut
        do i = 1, imut
          prcp(i,j) = real(dat4(i,jmut-j+1),4) ! YREV
        end do
      end do

      do j = 1, jmut
        do i = 1, imut
          if (prcp(i,j) == undef_gpcc) then
            prcp(i,j) = undef_mxe
          end if
        end do
      end do
