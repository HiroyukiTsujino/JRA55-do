...
!-----------------------------------------------------------------
  ...
  do k = 1, klev
    do j = 1, jmax
      do i = 1, imax
        if (nvalid_cl(i,j,k) == mday_total) then
          dim3_cl(i,j,k) = dim3_cl(i,j,k) / real(nvalid_cl(i,j,k),8)
          write(6,*) dim3_cl(i,j,k)
        else
  ...
  write(6,*) mday_total, mday_total/365.25, ' years of data'
  ...
  do j = 1, jmax
    do i = 1, imax
      if (dim3_cl(i,j,1) /= real(undef_out,8)) then
        write(6,*) dim3_cl(i,j,1)
      end if
    end do
  end do
