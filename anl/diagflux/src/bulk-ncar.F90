...
!$omp & zetaq, psi_mq, psi_hq, satmosu, qatmosu, zrough, rhoair, cpa, cdn_prv, test)
  do j = 1, jmx
    do i = 1, imx
      ...
      if (atexl(i,j) == 1.0d0) then
        ...
        dqr = qs - qatmos
        !
        wv = wdv(i,j) ! when input data is in MKS
