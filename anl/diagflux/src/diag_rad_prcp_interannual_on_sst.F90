...
program diagnosis_downward_flux
  ...
  type(type_libmxe_para) :: sstp, radp, pcpp
  type(type_libmxe_grid) :: sstg, radg, pcpg
  ...
  open(lun,file='namelist.diag_radpcp',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.diag_radpcp'
    stop
  end if
  ...
  if ( ios /= 0 ) then
    write(6,*) ' Read error : namelist.diag_radpcp', ios
    stop
  end if

  close(lun)

  if (l_lin_dswrf .and. l_lin_dlwrf .and. l_lin_precip) then
    l_need_table = .false.
  else
    l_need_table = .true.
  end if
