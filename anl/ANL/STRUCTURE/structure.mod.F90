...
module oc_structure
  ...
  !
  public :: set_hgrids
  public :: set_volt, set_volu, set_area_u, set_area_t
  !
...
subroutine read_topo(fltopo)
  ...
  do j=2, jmut
    do i=2, imut
      atexlbbl(i,j,1) = max(aexlbbl(i-1,j  ,1), aexlbbl(i,j  ,1),  &
          &               aexlbbl(i-1,j-1,1), aexlbbl(i,j-1,1)  )
    end do
  end do
  atexlbbl(1, 2:jmut,1) =atexlbbl(imut-3, 2:jmut,1)
  !
  !---------------------------------------
  !
