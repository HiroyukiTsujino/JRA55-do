...
program uv2wlwl
  ...
  namelist /nml_uv2wlwl/ flin_u, flin_v, flin_ssh, &
  ...
  do j = 2, jmut
    do i = 1, imut-1
      do k = 1, ksgm
        hl1 = (dzu(i, j  , k) + hu(i,j  ) * dsgm(k)) * u(i, j  , k)
        hl2 = (dzu(i, j-1, k) + hu(i,j-1) * dsgm(k)) * u(i, j-1, k)
        umo(i,j,k) = dy_br(i,j) * hl1 + dy_tr(i,j-1) * hl2
      end do
      do k = ksgm+1, kmax
        hl1 = dzu(i, j  , k) * u(i, j  , k)
        hl2 = dzu(i, j-1, k) * u(i, j-1, k)
        umo(i,j,k) = dy_br(i,j) * hl1 + dy_tr(i,j-1) * hl2
      end do
#ifdef OGCM_BBL
      hl1 = dzu(i, j  , km) * u(i, j  , km)
      hl2 = dzu(i, j-1, km) * u(i, j-1, km)
      umo(i,j,km) = dy_br(i,j) * hl1 + dy_tr(i,j-1) * hl2
#endif /* OGCM_BBL */
    end do
#ifdef OGCM_CYCLIC
  ...
  do j = 1, jmut - 1
    do i = 2, imut
      do k = 1, ksgm
        hl1 = (dzu(i  ,j,k) + hu(i  ,j) * dsgm(k)) * v(i  ,j,k)
        hl2 = (dzu(i-1,j,k) + hu(i-1,j) * dsgm(k)) * v(i-1,j,k)
        vmo(i,j,k) = dx_tl(i,j) * hl1 + dx_tr(i-1,j) * hl2
      end do
      do k = ksgm+1, kmax
        hl1 = dzu(i  ,j,k) * v(i  ,j,k)
        hl2 = dzu(i-1,j,k) * v(i-1,j,k)
        vmo(i,j,k) = dx_tl(i,j) * hl1 + dx_tr(i-1,j) * hl2
      end do
#ifdef OGCM_BBL
      hl1 = dzu(i  ,j,km) * v(i  ,j,km)
      hl2 = dzu(i-1,j,km) * v(i-1,j,km)
      vmo(i,j,km) = dx_tl(i,j) * hl1 + dx_tr(i-1,j) * hl2
#endif /* OGCM_BBL */
    end do
  end do
  !
