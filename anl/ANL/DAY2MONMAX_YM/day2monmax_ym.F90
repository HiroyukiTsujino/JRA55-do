...
program day2monmax_ym
  ...
  integer(4) :: i, j, k, n
  !
  namelist /nml_day2monmax_ym/ fltopo, iyear, imonth, fpathin, fpathout, tuxy, k_max, UNDEF
  !
  !==========================================
  !
  k_max=0
  !
  read(unit=5, nml=nml_day2monmax_ym)
  print *,'year      :', iyear
  print *,'month     :', imonth
    ...
    select case (tuxy)
      case('T') !-------------------------------------------------
          ...
          do d=1, daymonth(imonth)
            ...
            !
            do k=1, km
              where(dat3in(1:imut, 1:jmut, k) > dat3out(1:imut, 1:jmut, k))
                dat3out(1:imut, 1:jmut, k) = dat3in(1:imut, 1:jmut, k)
              end where
            end do
            !
            close(mtin)
