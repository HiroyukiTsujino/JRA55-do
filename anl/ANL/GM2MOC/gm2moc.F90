...
#endif /* OGCM_VARIABLE */
  !
  namelist /nml_gmmoc/   flin_u, flin_v, flin_ssh, flout, fltopo, flsclf, flibas
#ifdef OGCM_VARIABLE
...
subroutine around_the_world(jg, phi_std_degree)
  ...
  hl1 = 1.d0/hl1
  do k = 1, ksgm
    vstar = dx_tr(i00,j00)*dzu(i00,j00,k)*v(ic ,jc ,k)*ibm(i00,j00) &
  ...
  vstar = dx_tr(i00,j00)*dzu(i00,j00,km)*v(ic ,jc ,km)*ibm(i00,j00) &
  ...
  LOOP_AROUND_THE_WORLD : do while (ic < imut)
    ...
    if(min(sqphi02, sqphi01) >= sqphi20) then ! 等号が必要
      ...
      do k = 1, ksgm
        ustar = dy_tr(i00, j00)*dzu(i00, j00, k)*u(ic , jc , k)*ibm(i00,j00) &
...
#ifdef OGCM_BBL
      k=max(exnn(i00, j00), exnn(ic, jc))
      ustar = dy_tr(i00, j00)*dzu(i00, j00, km)*u(ic , jc , km)*ibm(i00,j00) &
      ...
      do k = 1, ksgm
        ustar = dy_br(i00, j00)*dzu(i00, j00, k)*u(i00, j00, k)*ibm(i00,j00) &
          &    +dy_tr(ic , jc )*dzu(ic , jc , k)*u(i00, j00, k)*ibm(ic ,jc )
      ...
      ustar = dy_tr(i00, j00)*dzu(i00, j00, km)*u(i00, j00, km)*ibm(i00,j00) &
        &    +dy_br(ic , jc )*dzu(ic , jc , km)*u(i00, j00, km)*ibm(ic ,jc )
      sv(jg,k) = sv(jg,k) +ustar
#endif /* OGCM_BBL */
      !
    end if
