...
!====================================================
program total_meridional_streamfunction
  !
  ...
  real(8), parameter :: lat_natl = 41.0d0
  real(8), parameter :: lat_matl = 26.5d0
  real(8), parameter :: lat_satl = -30.0d0
!  real(8), parameter :: lat_sos
!  real(8), parameter :: lat_son
  ...
  ! prognosticated velocity

  real(8)    :: u(imut, jmut, km)
  real(8)    :: v(imut, jmut, km)
  real(8),allocatable :: sv(:,:)     ! 緯度ごと層ごとの北向き流量     [cgs]

  ! diagnosed eddy-induced transport

  real(8)    :: u_gm(imut, jmut, km)
  real(8)    :: v_gm(imut, jmut, km)
  real(8),allocatable :: sv_gm(:,:)  ! 緯度ごと層ごとの北向き流量     [cgs]
  ...
  real(4),allocatable :: strmf_gm_g(:,:)  ! Global
  real(4),allocatable :: strmf_gm_a(:,:)  ! Atlantic
  real(4),allocatable :: strmf_gm_p(:,:)  ! Pacific
  real(4),allocatable :: strmf_gm_i(:,:)  ! Pacific

  real(4),allocatable :: strmf_out(:,:)  ! Pacific

  real(8) :: phi_std_degree_s, phi_std_degree_n 
  real(4) :: weight, tmpval
  real(4) :: strmf_tmp(km+1)
  ...
  character(len=256)    :: flin_gm_u ! 入力ファイル
  character(len=256)    :: flin_gm_v ! 入力ファイル
  character(len=256)    :: flout  ! 出力ファイル
  character(len=256)    :: flout_amoc  ! 出力ファイル
  ...
  logical :: l_inc_gm
  !
  namelist /nmlmoc/ flin_ssh, flin_u, flin_v, flin_gm_u, flin_gm_v, &
       &               flout, flout_amoc,                           &
       &               fltopo, flsclf, flibas, flstdout,            &
       &               jmgeo, slatg, dlatg, l_inc_gm
#ifdef OGCM_VARIABLE
  ...
  integer(4), parameter :: mtin_gm_u  = 82
  integer(4), parameter :: mtin_gm_v  = 83
  integer(4), parameter :: mtin_ssh = 84
  integer(4), parameter :: mtout    = 86
  integer(4), parameter :: mtout_amoc = 87
...
!  write(*,*) 'j_SOmin', j_SOmin
  ...
  print *,'flin_gm_u  :', trim(flin_gm_u)
  print *,'flin_gm_v  :', trim(flin_gm_v)
  print *,'flin_ssh   :', trim(flin_ssh)
  print *,'flout      :', trim(flout)
  print *,'flout_amoc :', trim(flout_amoc)
...
!  end do
  ...
  allocate(sv(1:jmgeo,1:km))
  allocate(sv_gm(1:jmgeo,1:km))

  allocate(strmf_g   (1:jmgeo,1:km+1))
  allocate(strmf_gm_g(1:jmgeo,1:km+1))
  allocate(strmf_a   (1:jmgeo,1:km+1))
  allocate(strmf_gm_a(1:jmgeo,1:km+1))
  allocate(strmf_p   (1:jmgeo,1:km+1))
  allocate(strmf_gm_p(1:jmgeo,1:km+1))
  allocate(strmf_i   (1:jmgeo,1:km+1))
  allocate(strmf_gm_i(1:jmgeo,1:km+1))

  allocate(strmf_out (1:jmgeo,1:km+1))
  ...
  ! read(mttmp, fmt="(i6,364i1)") jj,(ibas(i,j), i=1, imut)
    ...
    if(ios /= 0) write(*, *) 'reading error in file:', trim(flibas)
    if ( jj /= j ) then
...
!        if(m == 1 .or. m == 4) mask_atl(i, j) = 1 ! Atlantic or Mediterranean
  ...
  if (l_inc_gm) then
    open (mtin_gm_u, file=flin_gm_u, form='unformatted', &
         &  access='direct', recl=4*imut*jmut*km)
    open (mtin_gm_v, file=flin_gm_v, form='unformatted', &
         &  access='direct', recl=4*imut*jmut*km)
  end if
  ...
  open (mtout_amoc, file=flout_amoc, form='unformatted',&
    &  access='direct', recl=4*(km+1))
  irecw2=1
  ...
  if (l_inc_gm) then
    ! GM_U
    read (mtin_gm_u, rec=1) d3_r4
    u_gm(1:imut,1:jmut,1:km) = coefx(1:imut,1:jmut,1:km)*real(d3_r4(1:imut,1:jmut,1:km),8)
    u_gm(1:2,         1:jmut, 1:km) = u_gm(imut-3:imut-2, 1:jmut, 1:km)
    u_gm(imut-1:imut, 1:jmut, 1:km) = u_gm(3:4,           1:jmut, 1:km)

    ! GM_V
    read (mtin_gm_v, rec=1) d3_r4
    v_gm(1:imut,1:jmut,1:km) = coefy(1:imut,1:jmut,1:km)*real(d3_r4(1:imut,1:jmut,1:km),8)
    v_gm(1:2,         1:jmut, 1:km) = v_gm(imut-3:imut-2, 1:jmut, 1:km)
    v_gm(imut-1:imut, 1:jmut, 1:km) = v_gm(3:4,           1:jmut, 1:km)
  ...
  do jg=2, jmgeo
    ...
    call around_the_world_uv(jg, phi_std_degree)
  end do

  strmf_g(1:jmgeo, 1)=0.0
  do k=2, km+1
    do j=2, jmgeo
      strmf_g(j,k)=strmf_g(j,k-1)-real(sv(j,km+2-k), 4) * 1.e-12
  ...
  if (l_inc_gm) then
    sv_gm(1:jmgeo, 1:km)=0.d0

    do jg = 1, jmgeo
      phi_std_degree = slatg + dlatg*(jg-1)
      call around_the_world_gm(jg, phi_std_degree)
    end do

    strmf_gm_g(1:jmgeo, 1)=0.0
    do k = 2, km + 1
      do j = 2, jmgeo
        strmf_gm_g(j,k)=strmf_gm_g(j,k-1)-real(sv_gm(j,km+2-k), 4) * 1.e-12
        !    unit : cm3/s => Sv ( 10^6 m3/s )
      end do
    end do
  end if
  ...
  do k=2, km+1
    do j=2, jmgeo
      strmf_a(j,k)=strmf_a(j,k-1)-real(sv(j,km+2-k), 4) * 1.e-12
      !    unit : cm3/s => Sv ( 10^6 m3/s )
    end do
  end do
  !
  ...
  if (l_inc_gm) then
    ...
    !
    strmf_gm_a(1:jmgeo, 1)=0.0
    do k=2, km+1
      do j=2, jmgeo
        strmf_gm_a(j,k)=strmf_gm_a(j,k-1)-real(sv_gm(j,km+2-k), 4) * 1.e-12
        !    unit : cm3/s => Sv ( 10^6 m3/s )
      end do
      if (k==40) then
        do j = 2, jmgeo
          write(6,*) j, strmf_gm_a(j,k), sv_gm(j,km+2-k)
        end do
      end if
    end do
  end if
  ...
  do k=2, km+1
    do j=2, jmgeo
      strmf_p(j,k)=strmf_p(j,k-1)-real(sv(j,km+2-k), 4) * 1.e-12
      !    unit : cm3/s => Sv ( 10^6 m3/s )
    end do
  end do
  !
  ...
  if (l_inc_gm) then
    ...
    strmf_gm_p(1:jmgeo, 1)=0.0
    do k=2, km+1
      do j=2, jmgeo
        strmf_gm_p(j,k)=strmf_gm_p(j,k-1)-real(sv_gm(j,km+2-k), 4) * 1.e-12
        !    unit : cm3/s => Sv ( 10^6 m3/s )
      end do
    end do
  ...
  do k=2, km+1
    do j=2, jmgeo
      strmf_i(j,k)=strmf_i(j,k-1)-real(sv(j,km+2-k), 4) * 1.e-12
      !    unit : cm3/s => Sv ( 10^6 m3/s )
    end do
  end do
  !
  ...
  if (l_inc_gm) then
    ...
    !
    strmf_gm_i(1:jmgeo, 1)=0.0
    do k=2, km+1
      do j=2, jmgeo
        strmf_gm_i(j,k)=strmf_gm_i(j,k-1)-real(sv_gm(j,km+2-k), 4) * 1.e-12
        !    unit : cm3/s => Sv ( 10^6 m3/s )
      end do
    end do
  end if
  ...
  irecw=irecw+1
  strmf_out(1:jmgeo,1:km+1) = strmf_i(1:jmgeo, 1:km+1) + strmf_p(1:jmgeo, 1:km+1)
  write ( mtout, rec=irecw ) strmf_out(1:jmgeo, 1:km+1)
  irecw=irecw+1
  !----------------------------------------------------------
  write ( mtout, rec=irecw )  strmf_gm_g(1:jmgeo, 1:km+1)
  irecw=irecw+1
  write ( mtout, rec=irecw )  strmf_gm_a(1:jmgeo, 1:km+1)
  irecw=irecw+1
  write ( mtout, rec=irecw )  strmf_gm_p(1:jmgeo, 1:km+1)
  irecw=irecw+1
  strmf_out(1:jmgeo,1:km+1) = strmf_gm_i(1:jmgeo, 1:km+1) + strmf_gm_p(1:jmgeo, 1:km+1)
  write ( mtout, rec=irecw ) strmf_out(1:jmgeo, 1:km+1)
  irecw=irecw+1
  !---------------------------------------------------
  ...
  do jg = 1, jmgeo - 1
    ...
    if ((phi_std_degree_s <= lat_natl) .and. (lat_natl < phi_std_degree_n)) then
      write(6,*) jg, phi_std_degree_s, phi_std_degree_n
      weight = (lat_natl - phi_std_degree_s) / dlatg
      strmf_tmp(1:km+1) = (1.0d0 - weight) * (strmf_gm_a(jg, 1:km+1) + strmf_a(jg, 1:km+1)) &
           & + weight * (strmf_gm_a(jg+1, 1:km+1) + strmf_a(jg+1, 1:km+1))
      tmpval = maxval(strmf_tmp(1:km+1)) 
      write(mtstdout, '(a, f9.2, a)') ' North Atlantic max      :', &
           &  tmpval, ' [Sv]'
      write(mtout_amoc,rec=irecw2) strmf_tmp
      irecw2 = irecw2 + 1
      exit
    end if
  end do
  ...
  do jg=1, jmgeo - 1
    phi_std_degree_s = slatg + dlatg * (jg-1)
    phi_std_degree_n = slatg + dlatg * jg
    if ((phi_std_degree_s <= lat_matl) .and. (lat_matl < phi_std_degree_n)) then
      write(6,*) jg, phi_std_degree_s, phi_std_degree_n
      weight = (lat_matl - phi_std_degree_s) / dlatg
      strmf_tmp(1:km+1) = (1.0d0 - weight) * (strmf_gm_a(jg, 1:km+1) + strmf_a(jg, 1:km+1)) &
           & + weight * (strmf_gm_a(jg+1, 1:km+1) + strmf_a(jg+1, 1:km+1))
      tmpval = maxval(strmf_tmp(1:km+1)) 
      write(mtstdout, '(a, f9.2, a)') ' Subtropical Atlantic max:', &
           &  tmpval, ' [Sv]'
      write(mtout_amoc,rec=irecw2) strmf_tmp
      irecw2 = irecw2 + 1
      exit
  ...
  do jg=1, jmgeo - 1
    phi_std_degree_s = slatg + dlatg * (jg-1)
    phi_std_degree_n = slatg + dlatg * jg
    if ((phi_std_degree_s <= lat_satl) .and. (lat_satl < phi_std_degree_n)) then
      write(6,*) jg, phi_std_degree_s, phi_std_degree_n
      weight = (lat_satl - phi_std_degree_s) / dlatg
      strmf_tmp(1:km+1) = (1.0d0 - weight) * (strmf_gm_a(jg, 1:km+1) + strmf_a(jg, 1:km+1)) &
           & + weight * (strmf_gm_a(jg+1, 1:km+1) + strmf_a(jg+1, 1:km+1))
      tmpval = maxval(strmf_tmp(1:km+1)) 
      write(mtstdout, '(a, f9.2, a)') ' South Atlantic max      :', &
           &  tmpval, ' [Sv]'
      write(mtout_amoc,rec=irecw2) strmf_tmp
      irecw2 = irecw2 + 1
      exit
  ...
  deallocate(strmf_out)
  deallocate(strmf_i, strmf_gm_i)
  deallocate(strmf_p, strmf_gm_p)
  deallocate(strmf_a, strmf_gm_a)
  deallocate(strmf_g, strmf_gm_g)
...
!
!====================================================
subroutine around_the_world_uv(jg, phi_std_degree)
  !
...
!  write(6,*) 'around the world start', jg, j00, phi_std_degree
  ...
  LOOP_AROUND_THE_WORLD : do while (ic < imut)
    ...
    !
    call mp2lp(lambda00, phi00, mu0, psi0)
    call mp2lp(lambda20, phi20, mu2, psi0) ! e
    call mp2lp(lambda02, phi02, mu0, psi2) ! n
    call mp2lp(lambda01, phi01, mu0, psi1) ! s
    !
  ...
  !
end subroutine around_the_world_uv
!====================================================
!====================================================
subroutine around_the_world_gm(jg, phi_std_degree)
  !
  integer(4), intent(IN) :: jg
  real(8),    intent(IN) :: phi_std_degree
  !
...
!  j00=j00-1
  ...
  hl1 = 1.d0/hl1
  do k = 1, ksgm
    vstar = dx_tr(i00,j00)*dzu(i00,j00,k)*v_gm(ic ,jc ,k)*ibm(i00,j00) &
      &    +dx_tl(ic ,jc )*dzu(ic ,jc ,k)*v_gm(ic ,jc ,k)*ibm(ic ,jc )
    vstar = vstar*(thcksgm+hl1*(dy_bl(ic, jc)*ssh(i00+1, j00)+dy_tl(ic, jc)*ssh(i00+1, j00+1)))*thcksgmr
    sv_gm(jg,k)=sv_gm(jg,k) +vstar
  end do
  do k = ksgm+1, kmax
    vstar = dx_tr(i00,j00)*dzu(i00,j00,k)*v_gm(ic ,jc ,k)*ibm(i00,j00) &
      &    +dx_tl(ic ,jc )*dzu(ic ,jc ,k)*v_gm(ic ,jc ,k)*ibm(ic ,jc )
...
#ifdef OGCM_BBL
  k=max(exnn(i00,j00), exnn(ic,jc))
  vstar = dx_tr(i00,j00)*dzu(i00,j00,km)*v_gm(ic ,jc ,km)*ibm(i00,j00) &
    &    +dx_tl(ic ,jc )*dzu(ic ,jc ,km)*v_gm(ic ,jc ,km)*ibm(ic ,jc )
  sv_gm(jg,k)=sv_gm(jg,k)+vstar
#endif /* OGCM_BBL */
  !
  jend = j00  !  terminater
  ...
  LOOP_AROUND_THE_WORLD : do while (ic < imut)
    ...
    if(min(sqphi02, sqphi01) >= sqphi20) then ! 等号が必要
      ...
      hl1 = 1.d0 /(dx_bl(ic, jc)+dx_br(ic, jc))
      do k = 1, ksgm
        ustar = dy_tr(i00, j00)*dzu(i00, j00, k)*u_gm(ic , jc , k)*ibm(i00,j00) &
          &    +dy_br(ic , jc )*dzu(ic , jc , k)*u_gm(ic , jc , k)*ibm(ic ,jc )
        ustar = ustar*(thcksgm+hl1*(dx_bl(ic, jc)*ssh(i00, j00+1)+dx_br(ic, jc)*ssh(i00+1, j00+1)))*thcksgmr
        sv_gm(jg,k) = sv_gm(jg,k) - ustar
      end do
      do k = ksgm+1, kmax
        ustar = dy_tr(i00, j00)*dzu(i00, j00, k)*u_gm(ic , jc , k)*ibm(i00,j00) &
          &    +dy_br(ic , jc )*dzu(ic , jc , k)*u_gm(ic , jc , k)*ibm(ic ,jc )
...
#ifdef OGCM_BBL
      k=max(exnn(i00, j00), exnn(ic, jc))
      ustar = dy_tr(i00, j00)*dzu(i00, j00, km)*u_gm(ic , jc , km)*ibm(i00,j00) &
        &    +dy_br(ic , jc )*dzu(ic , jc , km)*u_gm(ic , jc , km)*ibm(ic ,jc )
      sv_gm(jg,k) = sv_gm(jg,k) - ustar
#endif /* OGCM_BBL */
      !
    else
      ...
      hl1 = 1.d0 /(dx_bl(i00, j00)+dx_br(i00, j00))
      do k = 1, ksgm+1
        ustar = dy_br(i00, j00)*dzu(i00, j00, k)*u_gm(i00, j00, k)*ibm(i00,j00) &
          &    +dy_tr(ic , jc )*dzu(ic , jc , k)*u_gm(i00, j00, k)*ibm(ic ,jc )
        ustar = ustar*(thcksgm+hl1*(dx_bl(i00, j00)*ssh(i00, j00)+dx_br(i00, j00)*ssh(i00+1, j00)))*thcksgmr
        sv_gm(jg,k) = sv_gm(jg,k) + ustar
      end do
      do k = ksgm+1, kmax
...
#ifdef OGCM_BBL
      k=max(exnn(i00, j00), exnn(ic, jc))
      ustar = dy_tr(i00, j00)*dzu(i00, j00, km)*u_gm(i00, j00, km)*ibm(i00,j00) &
        &    +dy_br(ic , jc )*dzu(ic , jc , km)*u_gm(i00, j00, km)*ibm(ic ,jc )
      sv_gm(jg,k) = sv_gm(jg,k) + ustar
#endif /* OGCM_BBL */
      !
    end if
  ...
  !
end subroutine around_the_world_gm
!====================================================
end program total_meridional_streamfunction
