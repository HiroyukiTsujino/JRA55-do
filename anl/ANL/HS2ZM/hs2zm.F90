...
subroutine around_the_world(jg, phi_std_degree)
  ...
  !
  do k = 1, ksgm
    vol_ts = a_br(i00-1,j00)*dzu(i00-1,j00,k)*ibm(i00-1,j00)  &
    ...
    vol_ts = vol_ts * (thcksgm+ssh(i00, j00))*thcksgmr
    !
...
#ifdef OGCM_BBL
  k = ktbtm(i00,j00)
  vol_ts = a_br(i00-1,j00)*dzu(i00-1,j00,km)*ibm(i00-1,j00)  &
    ...
    !
  vol_tstotal(jg,k) = vol_tstotal(jg,k) + vol_ts
  t_v(jg, k) = t_v(jg, k) + t(i00, j00, km) * vol_ts
#endif /* OGCM_BBL */
  !
  jend = j00  !  terminater
  !
