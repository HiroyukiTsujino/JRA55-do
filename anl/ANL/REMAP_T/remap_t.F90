...
program remap_scalar
  ...
  read (mtin, rec=m_rec) d2_r4
  if (tuw == 't' .or. tuw == 'w') then
    scalar(:,:) = atexl(:,:,k_mask)*real(d2_r4(:,:),8)
  else
    scalar(:,:) = aexl(:,:,k_mask)*real(d2_r4(:,:),8)
  end if

  if (lcyclic) then
    scalar(1:2,         1:jmut)=scalar(imut-3:imut-2, 1:jmut)
    scalar(imut-1:imut, 1:jmut)=scalar(3:4,           1:jmut)
  end if
  ...
  allocate(scalar_received(1:imaxg+1,1:jmaxg))
  allocate(area_received(1:imaxg+1,1:jmaxg))
  allocate(basin_index  (1:imaxg+1,1:jmaxg))
  allocate(dat_out(1:imaxg,1:jmaxg))
  lrec_out = imaxg*jmaxg*4

  scalar_received(1:imaxg+1,1:jmaxg) = 0.0d0
  area_received(1:imaxg+1,1:jmaxg) = 0.0d0
  basin_index  (1:imaxg+1,1:jmaxg) = 0.0d0
...
!  end do
  ...
  do jjdv = 1, nydiv
    ...
    do iidv = 1, nxdiv
      ...
      if (exnn(i_div_u(iidv),j_div_u(jjdv)) >= k_mask) then ! U-point datum is valid
        ...
        if (tuw == 't' .or. tuw == 'w') then
          scalar_tmp = area_tmp * scalar(i_div_t(iidv),j_div_t(jjdv))
        else
          scalar_tmp = area_tmp * scalar(i_div_u(iidv),j_div_u(jjdv))
        end if
          ...
          if ((latg_boundary(j-1) < lat_trn) .and. (lat_trn <= latg_boundary(j))) then
            do i = 1, imaxg + 1
              if ((long_boundary(i-1) < lon_trn) .and. (lon_trn <= long_boundary(i))) then
                ...
                scalar_received(i,j) = scalar_received(i,j) + scalar_tmp
  ...
  scalar_received(1,1:jmaxg) = scalar_received(1,1:jmaxg) + scalar_received(imaxg+1,1:jmaxg)
  area_received(1,1:jmaxg) = area_received(1,1:jmaxg) + area_received(imaxg+1,1:jmaxg)
  basin_index(1,1:jmaxg) = max(basin_index(1,1:jmaxg),basin_index(imaxg+1,1:jmaxg))
  ...
  if (latg(jmaxg) == 90.0d0) then
    ...
    basin_tmp = 0.0d0
    do i = 1, imaxg
      scalar_tmp = scalar_tmp + scalar_received(i,jmaxg)
      area_tmp = area_tmp + area_received(i,jmaxg)
      basin_tmp = max(basin_tmp,basin_index(i,jmaxg))
    end do
    do i = 1, imaxg
      scalar_received(i,jmaxg) = scalar_tmp / real(imaxg,8)
      area_received(i,jmaxg) = area_tmp / real(imaxg,8)
      basin_index(i,jmaxg) = basin_tmp
    end do
  end if
  ...
  do j = 1, jmaxg
    do i = 1, imaxg
      if (area_received(i,j) > 0.0d0) then
        scalar_received(i,j) = scalar_received(i,j) / area_received(i,j)
      else
        scalar_received(i,j) = undef
      end if
    end do
  end do
  ...
  dat_out(1:imaxg,1:jmaxg) = real(scalar_received(1:imaxg,1:jmaxg),4)
