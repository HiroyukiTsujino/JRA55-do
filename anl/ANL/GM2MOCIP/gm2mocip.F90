...
program gm_moc_ip
  ...
  integer(4) :: rl_sep(4)
  integer(4) :: RLMAX
  real(8),allocatable :: rholev(:), rholev_half(:)
...
#endif /* OGCM_VARIABLE */
  !
  namelist /nml_gmmocip/ refpress, rho_sep, rho_int,           &
...
!====================================================
!
!  謖㍾ョ夂キッ蠎ヲ縺ァ蟇コヲ螻、縺斐→縺ォ蛹怜髄縺崎シク騾√ｒ遨咲ョ!
!====================================================
subroutine around_the_world_ip(jg, phi_std_degree)
  ...
  hl1 = 1.d0/hl1
  do k = 1, ksgm
    hl2 = dx_tr(i00,j00)*dzu(i00,j00,k)*v(ic ,jc ,k)*ibm(i00,j00) &
  ...
  do k = ksgm+1, kmax
    vstar(k) = dx_tr(i00,j00)*dzu(i00,j00,k)*v(ic ,jc ,k)*ibm(i00,j00) &
      &       +dx_tl(ic ,jc )*dzu(ic ,jc ,k)*v(ic ,jc ,k)*ibm(ic ,jc )
    rholatbnd(k)=hl1*(dy_bl(ic, jc)*rho(i00+1, j00, k)+dy_tl(ic, jc)*rho(i00+1, j00+1, k))
  end do
#ifdef OGCM_BBL
  k=max(exnn(i00,j00), exnn(ic,jc))
  vstar(k+1) = dx_tr(i00,j00)*dzu(i00,j00,km)*v(ic ,jc ,km)*ibm(i00,j00) &
  ...
  LOOP_AROUND_THE_WORLD : do while (ic < imut)
    ...
    if(min(sqphi02, sqphi01) >= sqphi20) then
      ...
      do k = 1, ksgm
        hl2 = dy_tr(i00, j00)*dzu(i00, j00, k)*u(ic , jc , k)*ibm(i00,j00) &
      ...
      do k = ksgm+1, kmax
        ustar(k) = dy_tr(i00, j00)*dzu(i00, j00, k)*u(ic , jc , k)*ibm(i00,j00) &
          &       +dy_br(ic , jc )*dzu(ic , jc , k)*u(ic , jc , k)*ibm(ic ,jc )
        rholatbnd(k) = hl1*(dx_bl(ic, jc)*rho(i00, j00+1, k)+dx_br(ic, jc)*rho(i00+1, j00+1, k))
      end do
#ifdef OGCM_BBL
      k=max(exnn(i00, j00), exnn(ic, jc))
      ustar(k+1) = dy_tr(i00, j00)*dzu(i00, j00, km)*u(ic , jc , km)*ibm(i00,j00) &
