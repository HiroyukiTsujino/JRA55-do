...
program fresh_water_content
  ...
  !
  real(8) :: hld, hl1, hl2
  real(8),parameter :: sal_ref = 35.0d0
  ...
  namelist /nml_local_fw/ fltopo, flsclf, flin, flin_ssh, &
    &                     flout,       &
    ...
    do j = 1, jmut
      do i = 1, imut
        if (atexl(i,j,k) == 1.0d0) then
          hl1 = real(dt4(i,j,k),8)
          dt8(i,j,k) = atexl(i,j,k) * (sal_ref - hl1) / sal_ref
        else
          dt8(i,j,k) = undef8
        end if
      end do
...
!  ssh(:,:) = real(ssh4(:,:),8)
  ...
  call l_integ(wrk0, wrk, lat_b, lat_e, lon_b, lon_e)
  ...
  wrk0 = wrk0 * 1.0d-6
  wrk(1:km) = wrk(1:km) * 1.0d-6

  write(*, *) ' Local Average 3D ', wrk0
  do k = 1, km
    write(*, *) 'level k= ', k, wrk(k)
  end do

!  do k = 1, km
!    write(mtout, rec=k) real(wrk(k), 4)
!  end do
  write(mtout, rec=1) real(wrk0, 4)
  close(mtout)
...
!
!====================================================
subroutine l_integ(aveval0, aveval, latstt, latend, lonstt, lonend)
  !
...
!  end if
  ...
  do k = 1, km
    write(*,*) k, 'integral :', aveval(k)
    write(*,*) k, 'volume   :', avol(k)
!    if(avol(k) > 0.d0) then
...
end subroutine l_integ
!====================================================
end program fresh_water_content
