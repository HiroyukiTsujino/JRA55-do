...
!subroutine calcoe
!  !
!  use oc_mod_modelpar, only : imu, jmu
!  use oc_mod_common, only : pd, pm
!#ifdef OGCM_BBL
!  use oc_mod_param, only : grav, unit_bar, rho0
!  use oc_mod_common, only : pdb
!  use oc_mod_modelpar, only : ibt, iet, dp
!  use oc_mod_boundary, only : ktbtm
!#endif /* OGCM_BBL */
!  !
!  implicit none
!  !
...
!  do j = 1,jmu-1
!    do i = 1, imu-1
!      pdb(i,j) = grav * unit_bar * 0.25D0         & 
!      & * ( dp(max(ktbtm(i,j  ),1))+dp(max(ktbtm(i+1,  j),1))   &
!      &    +dp(max(ktbtm(i,j+1),1))+dp(max(ktbtm(i+1,j+1),1)) ) &
!      & * rho0
!    enddo
!  enddo
!#ifdef OGCM_CYCLIC
!   do j = 1, jmu
!     pdb(iet+2,j) = pdb(ibt+1,j)
!   enddo
!#endif /* OGCM_CYCLIC */
!  !
!#endif /* OGCM_BBL */
...
!  do j = 1, jmu
!    do i = 1, imu
!      epb(1,i,j) = dc_e(1) + pdb(i,j)*(dc_g(1)+pdb(i,j)*dc_h(1))
!      epb(2,i,j) = dc_e(2) + pdb(i,j)*(dc_g(2)+pdb(i,j)*dc_h(2))
!      epb(3,i,j) = dc_e(3) + pdb(i,j)*(dc_g(3)+pdb(i,j)*dc_h(3))
!      epb(4,i,j) = dc_e(4) + pdb(i,j)*dc_g(4)
!      epb(5,i,j) = dc_e(5)
!    end do
!  enddo
!  do j = 1, jmu
!    do i = 1, imu
!      fpb(1,i,j) = dc_f(1) + pdb(i,j)*(dc_g(5)+pdb(i,j)*dc_h(4))
!      fpb(2,i,j) = dc_f(2) + pdb(i,j)*(dc_g(6)+pdb(i,j)*dc_h(5))
!      fpb(3,i,j) = dc_f(3) + pdb(i,j)*(dc_g(7)+pdb(i,j)*dc_h(6))
!      fpb(4,i,j) = dc_f(4)
!      fpb(5,i,j) = dc_f(5) + pdb(i,j)*dc_g(8)
!      fpb(6,i,j) = dc_f(6)
!      fpb(7,i,j) = dc_f(7)
!    end do
!  enddo
