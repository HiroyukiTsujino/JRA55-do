...
module power
  ...
  integer,save :: im, jm, km, nm, nm2, reclen
  real,allocatable         :: r(:,:,:)
...
subroutine ini
  ...
  namelist /nml_power/ file_base_in, file_base_out, &
            & calc_i, calc_j, calc_k, l2d, &
            & nrec_first, nrec_last

  read( 5, nml=nml_power, iostat=i )
  if ( i /= 0 ) then
...
subroutine write_result
  ...
  open( lun, file=trim(file_base_out)//'.gd', form='unformatted', &
       & access='direct', recl=nm2*4+4, action='write' )
    write(lun,rec=1) real(p(0:nm2))
  close(lun)

  write(*,*)
  write(*,*) 'Output ('//trim(file_base_out)//'.gd) is created,'
  write(*,*) ' but grads control file is not made automatically.'
  write(*,*) '  direct access, single, length:',nm2+1
