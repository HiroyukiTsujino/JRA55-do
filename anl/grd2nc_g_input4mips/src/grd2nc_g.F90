...
!    nrecs = nrecs_out
  ...
  allocate(var_out_2d(1:nlons,1:nlats,1:num_var_all))
    ...
    do m = 1, nrecs_out
          ...
          do j = 1, nlats
            do i = 1, nlons
              if ( dat4(i,j,1) /= undef_in(n) ) then
                var_out_2d(i,j,n) = min(max(dat4(i,j,1),rmin(n)),rmax(n))
              else
                var_out_2d(i,j,n) = undef_out(n)
              end if
            end do
          end do
        else
      ...
      if (l_mask_out) then
        ...
        if (l_space_2d) then
          do j = 1, nlats
            do i = 1, nlons
              var_out_2d(i,j,num_var_out+1) = mask(i,j,1)
              var_out_2d(i,j,num_var_out+2) = volm(i,j,1)
            end do
          end do
        else
          do k = 1, nlvls
