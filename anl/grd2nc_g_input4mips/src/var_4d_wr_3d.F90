...
module netCDF_write
  ...
  character(len=64),parameter :: LVLBND_NAME='depth_coord_bnds'
  character(len=64),parameter :: LATBND_NAME='latitude_bnds'
  character(len=64),parameter :: LONBND_NAME='longitude_bnds'
  character(len=64),parameter :: BNDNAT_DIM_NAME='bnds_dim'
  integer(4),parameter :: NBNDNATDIM = 2
...
contains
  ...
  subroutine netCDF_write__create_file (                &
    ...
    real(8),   intent(in) :: lats_bnds(2,NLATS), lons_bnds(2,NLONS), lvls_bnds(2,NLVLS)
    ...
    integer(4) :: lvl_dimid, lon_dimid, lat_dimid, rec_dimid
    integer(4) :: bnd_dimid, bndnat_dimid

    !     The start and count arrays will tell the netCDF library where to
    !     write our data.

    integer(4) :: lon_varid, lat_varid, lvl_varid, rec_varid, bnd_varid
    integer(4) :: lonbnd_varid, latbnd_varid, lvlbnd_varid
    integer(4) :: dimids(NDIMS)

    integer(4) :: dimids_bnd(NBNDDIM)
    integer(4) :: dimids_bndnat(NBNDNATDIM)
    ...
    retval = nf_def_dim(ncid, BNDNAT_DIM_NAME, NBNDNATDIM, bndnat_dimid)
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    dimids_bndnat(1) = bndnat_dimid
    dimids_bndnat(2) = lat_dimid

    write(6,*) dimids_bndnat(1),dimids_bndnat(2)
    retval = nf_def_var(ncid, LATBND_NAME, NF_DOUBLE, NBNDNATDIM, dimids_bndnat, latbnd_varid)
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    dimids_bndnat(1) = bndnat_dimid
    dimids_bndnat(2) = lon_dimid

    write(6,*) dimids_bndnat(1),dimids_bndnat(2)
    retval = nf_def_var(ncid, LONBND_NAME, NF_DOUBLE, NBNDNATDIM, dimids_bndnat, lonbnd_varid)
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    dimids_bndnat(1) = bndnat_dimid
    dimids_bndnat(2) = lvl_dimid

    write(6,*) dimids_bndnat(1),dimids_bndnat(2)
    retval = nf_def_var(ncid, LVLBND_NAME, NF_DOUBLE, NBNDNATDIM, dimids_bndnat, lvlbnd_varid)
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    if (retval .ne. nf_noerr) call handle_err(retval)
    retval = nf_put_att_text(ncid, lat_varid, "bounds", 13, "latitude_bnds")
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    if (retval .ne. nf_noerr) call handle_err(retval)
    retval = nf_put_att_text(ncid, lon_varid, "bounds", 14, "longitude_bnds")
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    if (retval .ne. nf_noerr) call handle_err(retval)
    retval = nf_put_att_text(ncid, lvl_varid, "bounds", 16, "depth_coord_bnds")
    if (retval .ne. nf_noerr) call handle_err(retval)
    ...
    retval = nf_put_var_double(ncid, latbnd_varid, lats_bnds)
    if (retval .ne. nf_noerr) call handle_err(retval)

    retval = nf_put_var_double(ncid, lon_varid, lons)
    if (retval .ne. nf_noerr) call handle_err(retval)

    retval = nf_put_var_double(ncid, lonbnd_varid, lons_bnds)
    if (retval .ne. nf_noerr) call handle_err(retval)

    retval = nf_put_var_double(ncid, lvl_varid, lvls)
    if (retval .ne. nf_noerr) call handle_err(retval)

    retval = nf_put_var_double(ncid, lvlbnd_varid, lvls_bnds)
    if (retval .ne. nf_noerr) call handle_err(retval)
  ...
  !----------------------------------------------------------------------------
  !-----------------------------------------------------------------------------
  subroutine netCDF_write__all_at_once  (               &
       & var_out, nrecs_out,                            &
       & nrecs, recs, recsb, recse,                     &
       & FILE_NAME,                                     &
       & nvars, VAR_NAME,                               &
    ...
    integer(4),intent(in) :: nvars
    real(4),   intent(in) :: var_out(NLONS, NLATS, NLVLS, nrecs, nvars)
    real(8),   intent(in) :: recs(nrecs), recsb(nrecs), recse(nrecs)
    character(len=*),intent(in) :: FILE_NAME
    character(len=*),intent(in) :: VAR_NAME(nvars)
    ...
    do n = 1, nvars
      ...
      retval = nf_put_vara_real(ncid, var_varid(n), start, count, var_out(1,1,1,1,n))
      if (retval .ne. nf_noerr) call handle_err(retval)
      write(6,*) 'writing data O.K.'
  ...
  end subroutine netCDF_write__all_at_once
  !----------------------------------------------------------------------------
