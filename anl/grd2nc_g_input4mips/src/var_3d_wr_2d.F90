...
module netCDF_write_3d
  ...
  !     We are writing 3D data, a lon-lat grid, with 2
  !     timesteps of data.
...
contains
  !----------------------------------------------------------------------------
  subroutine netCDF_write_3d__create_file ( &
       & NLONS, NLATS,                      &
       & lats, lons,                        &
       & lats_bnds, lons_bnds,              &
       & FILE_NAME,                         &
    ...
    real(8),   intent(in) :: lats(NLATS), lons(NLONS)
    real(8),   intent(in) :: lats_bnds(2,NLATS), lons_bnds(2,NLONS)
    ...
    integer(4) :: lon_dimid, lat_dimid, rec_dimid, bnd_dimid, bndnat_dimid

    !     The start and count arrays will tell the netCDF library where to
    !     write our data.

    integer(4) :: lon_varid, lat_varid, rec_varid, bnd_varid
    integer(4) :: lonbnd_varid, latbnd_varid
  ...
  end subroutine netCDF_write_3d__create_file
  !-----------------------------------------------------------------------------
  !-----------------------------------------------------------------------------
  subroutine netCDF_write_3d__var_3d  (                 &
       & var_out,  irec,                                &
    ...
    integer(4),intent(in) :: NLATS, NLONS
    integer(4),intent(in) :: nvars
    real(4),   intent(in) :: var_out(NLONS, NLATS, nvars)
    ...
    do n = 1, nvars
      ...
      start(3) = irec
      retval = nf_put_vara_real(ncid, var_varid(n), start, count, var_out(1,1,n))
      if (retval .ne. nf_noerr) call handle_err(retval)
