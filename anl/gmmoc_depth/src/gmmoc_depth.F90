...
!   from GM bolus velocity
!
!====================================================
program gm_meridional_streamfunction
  ...
  namelist /nml_gmmoc_dep/ flin_u, flin_v, flin_ssh, flout, &
...
!        if(m == 1 .or. m == 4) mask_atl(i, j) = 1 ! Atlantic or Mediterranean
  ...
  u(1:imut,1:jmut,1:km) = coefx(1:imut,1:jmut,1:km)*real(d3_r4(1:imut,1:jmut,1:km),8)
  !
  ...
  !!!!!write(*,*) 'minval u:', minval(u)
  !
  ! V
  !
  read (mtin_v, rec=1) d3_r4
  v(1:imut,1:jmut,1:km) = coefy(1:imut,1:jmut,1:km)*real(d3_r4(1:imut,1:jmut,1:km),8)
  !
  if (para%lcyclic) then
    v(1:2,         1:jmut, 1:km)=v(imut-3:imut-2, 1:jmut, 1:km)
    v(imut-1:imut, 1:jmut, 1:km)=v(3:4,           1:jmut, 1:km)
  end if
  !!!!!write(*,*) 'maxval v:', maxval(v)
  !!!!!write(*,*) 'minval v:', minval(v)
  !-------------------------
  !
  !  SSH
  !
...
subroutine around_the_world(jg, phi_std_degree, l_check)
  ...
  do k = 1, kmax
    vstar = dx_tr(i00,j00)*dzu(i00,j00,k)*v(ic ,jc ,k)*ibm(i00,j00) * use_prev &
      &    +dx_tl(ic ,jc )*dzu(ic ,jc ,k)*v(ic ,jc ,k)*ibm(ic ,jc ) * use_next
    sv(jg,k)=sv(jg,k) + vstar
  end do
  if (para%lbbl) then
    k=max(exnn(i00,j00), exnn(ic,jc))
    vstar = dx_tr(i00,j00)*dzu(i00,j00,km)*v(ic ,jc ,km)*ibm(i00,j00) * use_prev &
  ...
  LOOP_AROUND_THE_WORLD : do while (ic < imut)
    ...
    if(min(sqphi02, sqphi01) >= sqphi20) then ! 等号が必要
      ...
      if (para%ltripolar) then
        ...
        if ((hl1 == 1.0d0) .and. (jc == jmut-3)) then
          if (use_prev == 1.0d0) then ! proceed northward
            ...
            do k = 1, kmax
              ustar = dy_tr(i00,j00)*dzu(i00,j00,k)*u(i00,j00+1,k)*ibm(i00,j00) * use_prev
              sv(jg,k) = sv(jg,k) - ustar
            end do
            if (para%lbbl) then
              k = max(exnn(i00, j00), exnn(i00, j00+1))
              ustar = dy_tr(i00, j00)*dzu(i00, j00, km)*u(i00, j00+1, km)*ibm(i00, j00) * use_prev
              sv(jg,k) = sv(jg,k) - ustar
            end if
            ...
            do k = 1, kmax
              ustar = dy_tr(ic,jc)*dzu(ic,jc,k)*u(ic,jc+1,k)*ibm(ic,jc) * use_next
              sv(jg,k) = sv(jg,k) + ustar
            end do
            if (para%lbbl) then
              k = max(exnn(ic, jc), exnn(ic, jc+1))
              ustar = dy_tr(ic, jc)*dzu(ic, jc, km)*u(ic, jc+1, km)*ibm(ic, jc) * use_next
              sv(jg,k) = sv(jg,k) + ustar
            end if
          end if
          u_done(ic, jc)=1.d0
      ...
      if(u_done(ic,jc) == 1.0d0) then
        ...
        do k = 1, kmax
          ustar = dy_tr(i00, j00)*dzu(i00, j00, k)*u(ic , jc , k)*ibm(i00,j00) * use_prev
          sv(jg,k) = sv(jg,k) -ustar
        end do
        if (para%lbbl) then
          k=max(exnn(i00, j00), exnn(ic, jc))
          ustar = dy_tr(i00, j00)*dzu(i00, j00, km)*u(ic , jc , km)*ibm(i00,j00) * use_prev
          sv(jg,k) = sv(jg,k) -ustar
        end if
        exit LOOP_AROUND_THE_WORLD
      end if
      ...
      do k = 1, kmax
        ustar = dy_br(i00, j00)*dzu(i00, j00, k)*u(i00, j00, k)*ibm(i00,j00) * use_prev &
          &    +dy_tr(ic , jc )*dzu(ic , jc , k)*u(i00, j00, k)*ibm(ic ,jc ) * use_next
        sv(jg,k) = sv(jg,k) +ustar
      end do
      if (para%lbbl) then
        k=max(exnn(i00, j00), exnn(ic, jc))
        ustar = dy_br(i00, j00)*dzu(i00, j00, km)*u(i00, j00, km)*ibm(i00,j00) * use_prev &
             & +dy_tr(ic , jc )*dzu(ic , jc , km)*u(i00, j00, km)*ibm(ic ,jc ) * use_next
        sv(jg,k) = sv(jg,k) +ustar
      end if
      !
    end if
...
!====================================================
end program gm_meridional_streamfunction
