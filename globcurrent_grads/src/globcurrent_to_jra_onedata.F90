! -*-F90-*-
!
!----------------- globcurrent_to_jra_onedata.F90 ------------------
!
!  Information:
...
program horizontal_interpolation
  ...
  do k = 1, kmf
    ...
    where(dat4_org(1:imf,1:jmf) /= undef_in) &
         &  imask_org(1:imf,1:jmf,k) = imask_org(1:imf,1:jmf,k) + 1
    mask_org(1:imf,1:jmf,k) = real(imask_org(1:imf,1:jmf,k),8)
