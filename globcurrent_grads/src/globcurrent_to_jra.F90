...
program horizontal_interpolation
  ...
  integer(4),allocatable :: imask_org(:,:,:)
  ...
  character(len=clen) :: flin1, flin1_base
  character(len=clen) :: flot1, flot1_base
  character(len=clen) :: file_topo, file_mask
  character(len=clen) :: file_topo_gc

  integer(4) :: irec1, irec2
  integer(4) :: irecv, irecm
  ...
  integer(4) :: ibyr, ieyr, ibmn, iemn
  integer(4) :: ny, nm, ms, me

  !-----------------------------------------------------------------------

  namelist /nml_hintpol/ flin1_base, flot1_base, &
       & ibyr, ieyr, ibmn, iemn, &
       & undef_out, num_data, irecv, irecm, &
       & file_topo_gc, &
  ...
  allocate(imask_org(1:imf,1:jmf,1:kmf))
...
!  mask_org(imf+1:imf+2,1:jmf,1:kmf) = mask_org(1:2,1:jmf,1:kmf)
  ...
  LOOP_MONTH: do nm = ms, me

  write(flin1,'(1a,i4.4,i2.2)') trim(flin1_base),ny,nm
  write(6,*) 'DATA read from ',trim(flin1)
  open(mtin1,file=flin1,form='unformatted',access='direct',recl=4*imf*jmf)
  irec1 = 0

  write(flot1,'(1a,i4.4,i2.2)') trim(flot1_base),ny,nm
  ...
  do n = 1, num_data
    ...
    do k = 1, kmf
      ...
      irec1 = irec1 + 1
      read(mtin1,rec=irecv) dat4_org(1:imf,1:jmf)
      ...
      read(mtin1,rec=irecm) dat4_org(1:imf,1:jmf)

      imask_org(1:imf,1:jmf,k) = imask_org(1:imf,1:jmf,k) + nint(dat4_org(1:imf,1:jmf))
      mask_org(1:imf,1:jmf,k) = min(real(dat4_org(1:imf,1:jmf),8),1.0d0)
      mask_org(-1:0,1:jmf,k) = mask_org(imf-1:imf,1:jmf,k)
      mask_org(imf+1:imf+2,1:jmf,k) = mask_org(1:2,1:jmf,k)
        ...
        do i = 1, imt
          ...
          if ((mask_new(i,j,k) > 0.0d0) .and. (total_ocn == 4.0d0)) then
            ...
            dat_new(i,j,k) = (dist1 * dat_org(iss ,jss ,k)  &
                 ...
                 &          / total
            dat_avail(i,j,k) = 1.0d0
            if (abs(dat_new(i,j,k)) > 1.0d2) then
              write(6,*) ' data processing error '
              write(6,*) dat_org(iss ,jss ,k), mask_org(iss ,jss ,k)
              write(6,*) dat_org(issp,jss ,k), mask_org(issp,jss ,k)
              write(6,*) dat_org(iss ,jssp,k), mask_org(iss ,jssp,k)
              write(6,*) dat_org(issp,jssp,k), mask_org(issp,jssp,k)
              stop
            end if
          else
            if (mask_new(i,j,k) > 0.0d0) then
              dat_new(i,j,k) = 0.0d0
            else
    ...
    do j = 1, jmt
      do i = 1, imt
        if (dat_new(i,j,1) /= real(undef_out,8)) then
          mask_valid(i,j) = mask_valid(i,j) + 1.0
        end if
      end do
    end do
  ...
  if (l_mask_out) then
    ...
    write(6,*) ' topo file (glob current) = ',trim(file_topo_gc)
    call open_file_sequential(nf_out,file_topo_gc)
    write(nf_out) imask_org(:,:,1), imask_org(:,:,1)
    call close_file(nf_out)
