! -*-F90-*-
!==================================================================
program read_netcdf_output_grads_daily
  !----------------------------------------------------------------
  ! Information:
  ...
  character(256) :: flnin_globcurrent, flnot_base, flnin_dir, flnot_dir
  ...
  namelist /nml_globcurrent_daily/ &
       & flnin_dir, flnin_globcurrent, undef_in, &
       & flnot_dir, flnot_base, undef_out, &
       & iyst, iyed, mst, med
  ...
  open (11,file='namelist.globcurrent_daily')
  read (11,nml=nml_globcurrent_daily)
  close(11)
  ...
  LOOP_DAY: do id = 1, nded
      ...
      do j = 1, jmut
        do i = 1, imut/2 ! eastern longitude
          if (dat4(i+imut/2,j) /= undef_in) then
            numd(i,j,n) = 1
            svel(i,j,n) = real(dat4(i+imut/2,j),4)
          end if
        end do
        do i = 1, imut/2 ! western longitude
          if (dat4(i,j) /= undef_in) then
            numd(i+imut/2,j,n) = 1
            svel(i+imut/2,j,n) = real(dat4(i,j),4)
          end if
        end do
      end do
    ...
    do j = 1, jmut
      do i = 1, imut
        if ((numd(i,j,1) /= 1) .or. (numd(i,j,2) /= 1)) then
    ...
    write(flot1,'(1a,a1,i4.4,a1,1a,a1,i4.4,i2.2,i2.2)') &
         & trim(flnot_dir),'/',iy,'/',trim(flnot_base),'.',iy,im,id
    write(6,*) 'Surface current written to ',trim(flot1)
    open (mtot1,file=trim(flot1),form='unformatted',access='direct',convert='big_endian',recl=4*imut*jmut)
...
end program read_netcdf_output_grads_daily
