...
#RDIR=/mwork/tsuzuki/CaMa_out/$EXP         #   directory to run CaMa-Floo
...
##### Simulation Time #################
#YSTART=1958                           #   start year (from YSTART / Jan / 1st )
#YEND=2016
#SPINUP=2                              #   1 for restart, 2 for spinup
#NSP=2                                 #   spinup years

YSTART=${start_year}                           #   start year (from YSTART / Jan / 1st )
YEND=${end_year}
SPINUP=1                              #   1 for restart, 2 for spinup
NSP=0                                 #   spinup years

# CRESTSTO="set-by-shell"               #   restart file name

##### Map & Topography ################
##FMAP="${BASE}/map/JRA55_15min"       #   map directory

FMAP="${BASE}/map/JRA55_rivmap_160805"

# FMAP="${BASE}/map/region_15min"       # (regional map) 

CDIMINFO="${FMAP}/diminfo.txt"   #   dimention info (1deg, 0E->360E, 90N-90S)
...
CGRAREA=${FMAP}/grarea_grid.bin            #   unit-catchment area [m2]
...
CRIVWTH=${FMAP}/rivwth_v2.bin            #   channel width       [m] (empirical power-low)
# CRIVWTH=${FMAP}/rivwth_gwdlr.bin      #   channel width       [m] (GWD-LR + filled with empirical)
CRIVHGT=${FMAP}/rivhgt_v2.bin            #   channel depth       [m] (empirical power-low)
...
CINPMAT=${FMAP}/inpmat.bin            #   runoff input matrix (1deg, 0E->360E, 90N-90S)     !! for sample bonary input
...
#CROFDIR="${BASE}/inp/Runoff/RIVGRID_dy_tfact2_rivmap160805/"   #   runoff directory
CROFDIR="/home/htsujino/CaMa-Flood/INPUT/"
CROFPRE="rof"                         #   runoff prefix/suffix  ( $(PREFIX)yyyymmdd$(SUFFIX) )
CROFSUF=".grd"
...
CRIVDPHDIR="$COUTDIR"                 #   river water depth       [m]

CFLDOUTDIR="NONE"                     #   floodplain discharge    [m3/s]
CFLDSTODIR="NONE"                     #   floodplain storage      [m]
CFLDDPHDIR="$COUTDIR"                 #   floodplain water depth  [m]
CFLDAREDIR="$COUTDIR"                 #   flooded area            [m]
CFLDFRCDIR="$COUTDIR"                 #   flooded area fraction   [m2/m2]

CSFCELVDIR="$COUTDIR"                 #   water surface elevation [m]
COUTFLWDIR="$COUTDIR"                 #   total discharge (rivout+fldout)   [m3/s]
CSTORGEDIR="$COUTDIR"                 #   total storage (rivsto+fldsto)     [m3]

CPTHOUTDIR="$COUTDIR"                 #   net bifurcation flow (grid-based) [m3/s]
CPTHFLWDIR="$COUTDIR"                 #   bifurcation flow (channel based)  [m3/s]
