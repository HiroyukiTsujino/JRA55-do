!-*-F90-*-
program read_qscat3_and_output_daily
!====================================================================
!     Filename: qscat2daily.F90
!
!     Usage: 
!
...
!-------------------------------------------------------------------
  ...
  namelist /nml_qscat_daily/ l3_file, file_out, undef_out
...
!     Read the input filename.
  ...
  write(*,*)
  if (l3_file .eq. 'no_file') then
    write(6,*) 'Usage: read_qscat3 <Level 3 file>'
    stop
  endif
  if (l3_file .eq. 'missing_file') then
...
!     Make sure that the file is a QuikSCAT Level 3 file
  ...
  if (product.ne.'QSCATL3') then
    write(6,*) 'The input file is not a QuikSCAT Level 3 file'
...
!     the QuikSCAT SDS data in slabs, please refer to read_qscat2a.f.
  ...
  do j = 1, YGRID
    do i = 1, XGRID
      asc_valid(i,j) = 0.0
      if (asc_wvc_count(i,j) == 1.0) then
!        if ((asc_rain_flag(i,j) == 0.0) .or. (asc_rain_flag(i,j) == 4.0)) then
          asc_valid(i,j) = 1.0
!        end if
      end if
      des_valid(i,j) = 0.0
      if (des_wvc_count(i,j) == 1.0) then
!        if ((des_rain_flag(i,j) == 0.0) .or. (des_rain_flag(i,j) == 4.0)) then
          des_valid(i,j) = 1.0
!        end if
      end if
    end do
  ...
  do j = 1, YGRID
    do i = 1, XGRID
      ...
      if (total_valid > 0.0) then
        swind(i,j) = (asc_valid(i,j) * asc_avg_wind_speed(i,j) &
             &      + des_valid(i,j) * des_avg_wind_speed(i,j)) / total_valid
        uwind(i,j) = (asc_valid(i,j) * asc_avg_wind_vel_u(i,j) &
             &      + des_valid(i,j) * des_avg_wind_vel_u(i,j)) / total_valid
        vwind(i,j) = (asc_valid(i,j) * asc_avg_wind_vel_v(i,j) &
             &      + des_valid(i,j) * des_avg_wind_vel_v(i,j)) / total_valid
      else
        swind(i,j) = undef_out
        uwind(i,j) = undef_out
        vwind(i,j) = undef_out
...
end program read_qscat3_and_output_daily
