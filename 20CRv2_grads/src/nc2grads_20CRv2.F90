...
program read_netcdf_output_grads
  ...
  integer(4), parameter :: start_year = 1851
  ...
  real(4),parameter :: undef_20CRv2 = -9.96921e36
  ...
  open (10,file='namelist.20CRv2_surf')
  read (10,nml=nml_20CRv2_surf)
  close(10)
  ...
  do nyear = ibyr, ieyr
    do m = 1, 12
      ...
      do j = 1, jmut
        do i = 1, imut
          qair(i,j) = real(dat4(i,jmut-j+1),4) ! YREV
        end do
      end do

      do j = 1, jmut
        do i = 1, imut
          if (qair(i,j) == undef_20CRv2) then
            qair(i,j) = undef_mxe
          end if
        end do
      end do
