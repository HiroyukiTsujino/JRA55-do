...
!-----------------------------------------------------------------
  ...
  do month = 1, 12
    ...
    do i = 1, total_grid_1d
      if (mask_wind(i) /= 0.0d0) then
        if ((swind_obs(i) == real(undef_obs,8)) .and. (swind_jra(i) == real(undef_jra,8))) then
          ...
          corr_mag(i) = swind_obs(i) / swind_jra(i)
        end if
      else
    ...
    write(mtot1,rec=month) real(corr_mag(1:total_grid_1d),4)
    ...
    write(mtot2,rec=6*(month-1)+1) real(corr_rot     (1:total_grid_1d),4)
    write(mtot2,rec=6*(month-1)+2) real(angle_obs    (1:total_grid_1d),4)
    write(mtot2,rec=6*(month-1)+3) real(angle_jra    (1:total_grid_1d),4)
    write(mtot2,rec=6*(month-1)+4) real(corr_rot_org (1:total_grid_1d),4)
    write(mtot2,rec=6*(month-1)+5) real(swind_vec_obs(1:total_grid_1d),4)
    write(mtot2,rec=6*(month-1)+6) real(swind_vec_jra(1:total_grid_1d),4)
    ...
    write(mtot3,rec=month) real(data_mag_latlon,4)
    write(mtot4,rec=month) real(data_rot_latlon,4)
