! -*-F90-*-
program make_monthly_correction_sph2m
  ...
  logical :: l_apply_filter
  real(8),allocatable :: mag_org(:)
  real(8),allocatable :: mag_flt(:)
  real(8) :: hl1, hl2
  integer(4) :: iw, ie
  ...
  open (lun,file='namelist.make_sph2mcorrec')
  read (lun,nml=nml_make_sph2mcorrec)
  close(lun)
  ...
  do month = 1, 12
    ...
    if (l_apply_filter) then
      ...
      do j = 1, jmut
        ...
        allocate(mag_org(1:num_xgrid(j)))
        allocate(mask_org(1:num_xgrid(j)))
        allocate(mag_flt(1:num_xgrid(j)))

        mag_org(1:num_xgrid(j)) = data_correc(ibgn:iend)
        ...
        if (num_xgrid(j) == imut) then ! not reduced
          do i = 1, num_xgrid(j)
            ...
            if (hl1 == 4.0d0) then
              hl2 = mask_org(iw) * mag_org(iw) + mask_org(ie) * mag_org(ie) &
                   & + 2.0d0 * mask_org(i) * mag_org(i)
              mag_flt(i) = hl2 / hl1
            else
              mag_flt(i) = mag_org(i)
            end if
        ...
        data_correc(ibgn:iend) = mag_flt(1:num_xgrid(j))
...
end program make_monthly_correction_sph2m
