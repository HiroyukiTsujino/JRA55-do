...
program calc_wind_ceof_monthly
  ...
  tanh_half = 1.0d2
  open (11,file='namelist.windcorrec_ceof_mon')
  ...
  do im = 1, 12
    ...
    write(mtot1,rec=im) real(corr_mag(1:total_grid_1d),4)
    write(mtot2,rec=3*(im-1)+1) real(corr_rot(1:total_grid_1d),4)
    write(mtot2,rec=3*(im-1)+2) real(corr_rot_org(1:total_grid_1d),4)
    write(mtot2,rec=3*(im-1)+3) real(ene_ratio(1:total_grid_1d),4)
    ...
    do j = 1, jmut
      ...
      if (num_xgrid(j) == imut) then
        ...
        data_org(1:num_xgrid(j)) = corr_rot(ibgn:iend)
        data_org(num_xgrid(j)+1) = corr_rot(ibgn)
    ...
    write(mtot3,rec=im) real(data_correc_latlon,4)
