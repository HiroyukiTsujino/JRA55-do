! -*-F90-*-
program make_monthly_correction_tmp2m
  ...
  real(8),allocatable :: data_ref_all(:)
  real(8),allocatable :: data_jra_all(:)
  ...
  character(256) :: file_ref_all, file_ref_all_base
  character(256) :: file_jra_all, file_jra_all_base
  ...
  namelist /nml_make_tmp2mcorrec_fill/ &
       &  file_mask,              &
       &  file_ref_base,          &
       &  file_jra_base,          &
       &  file_ref_all_base,      &
  ...
  l_apply_mask=.false.
  open (lun,file='namelist.make_tmp2mcorrec_fill')
  read (lun,nml=nml_make_tmp2mcorrec_fill)
  close(lun)
  ...
  allocate(data_ref_all(1:total_grid_1d))
  allocate(data_jra_all(1:total_grid_1d))
  ...
  open (lun1,file=file_correc,form='unformatted',access='direct', &
       & convert='little_endian',recl=4*total_grid_1d)
  ...
  do month = 1, 12
    ...
    open(lun,file=file_ref,form='unformatted',access='direct', &
         & convert='little_endian',recl=4*total_grid_1d)
    read(lun,rec=1) work4
    close(lun)
    ...
    open(lun,file=file_jra,form='unformatted',access='direct', &
         & convert='little_endian',recl=4*total_grid_1d)
    read(lun,rec=1) work4
    close(lun)
    ...
    write(file_ref_all,'(1a,i2.2)') trim(file_ref_all_base),month
    open(lun,file=file_ref_all,form='unformatted',access='direct', &
         & convert='little_endian',recl=4*total_grid_1d)
    read(lun,rec=1) work4
    close(lun)
    ...
    do n = 1, total_grid_1d
      if (work4(n) /= undef4_ref) then
        data_ref_all(n) = real(work4(n),8)
      else
        data_ref_all(n) = 0.0d0
      end if
    end do

    write(file_jra_all,'(1a,i2.2)') trim(file_jra_all_base),month
    open(lun,file=file_jra_all,form='unformatted',access='direct', &
         & convert='little_endian',recl=4*total_grid_1d)
    read(lun,rec=1) work4
    close(lun)
    ...
    do n = 1, total_grid_1d
      if (work4(n) /= undef4_jra) then
        data_jra_all(n) = real(work4(n),8)
      else
        data_jra_all(n) = 0.0d0
      end if
    end do
...
!        data_correc(n) = 0.0d0
        data_correc(n) = data_ref_all(n) - data_jra_all(n)
      end if
    end do
    ...
    if (l_apply_mask) then
      do n = 1, total_grid_1d
        if (data_mask(n) == 0.0d0) then
          data_correc(n) = real(undef8_out,8)
        end if
      end do
    end if
...
end program make_monthly_correction_tmp2m
