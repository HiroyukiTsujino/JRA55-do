...
ref_all_base=../linkdir/forcing/ensemble_monclim_TL319r
...
do
  if [ "${surf}" = "ice" ]; then
    ...
    ref_base=../linkdir/forcing/ensemble_monclim_TL319r
  fi
  sed -e s%@period_ref@%${period_ref}% \
      ...
      -e s%@surf@%${surf}% \
      namelist.make_tmp2mcorrec_fill_template > namelist.make_tmp2mcorrec_fill
