! -*-F90-*-
!------------- calib_tmp10m_iceedge_mon_interannual.F90 -----------------
!  Information:
  ...
  LOOP_SURF_FORCE: do while ( .true. )
    ...
    do j = 1, jmf
      do i = 1, imf
        ...
        else ! land grid : same as water surface
          hl1 = (g1 + g2 * satf(i,j)) / (1.0d0 + g3 * satf(i,j))
          hl2 = 10 ** hl1
          hl3 = 1.0d0 + 1.0d-6 * slpf(i,j) * (h1 + h2 * satf(i,j)**2)
          esat = hl2 * hl3
    ...
    do j = 1, jmf
      do i = 1, imf
        if ((icjf(i,j) < 0.5d0) .or. (icef(i,j) > 0.99d0))  then ! out of marginal sea ice zone
...
!        if ((icef(i,j) < 0.15d0) .or. (icef(i,j) > 0.99d0))  then
    ...
    ! finalize (smoothing one-grid around ice edges)
