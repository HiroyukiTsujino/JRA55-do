! -*-F90-*-
!------------- calib_tmp2m_antarc_interannual.F90 ------------------
!  Information:
!     Calibrate surface temperature and specific humidity
!-------------------------------------------------------------------
program calibration_surface_temperature_antarctica
...
!  real(8), parameter :: t_ubd = -5.0d0 ! upper bound of cut-off temperature, ad hoc modification by MRI
  real(8) :: rsec
  real(8) :: cosfactor
  ...
  namelist /nml_calibsat_antarc/ calc_start, calc_end, &
       &  l_leap_valid, &
       &  calib_base_dir, &
       &  fout_base_sat, &
       &  fout_base_sph
  ...
  open(lun,file='namelist.calibtmp10m_antarc',iostat=istat,status='old')

  if ( istat /= 0 ) then
    write(6,*) ' Cannot find file : namelist.calibtmp10m_antarc'
    stop
  end if
  ...
  rewind(lun)
  read(lun, nml=nml_calibsat_antarc, iostat=istat)
  ios = ios + istat

  if ( ios /= 0 ) then
    write(6,*) ' Read error : namelist.calibtmp10m_antarc', ios
    stop
  end if
  ...
  LOOP_SURF_FORCE: do while ( .true. )
    ...
    do j = 1, jmf
!      if (alatf(j) < -60.d0) then
      if (alatf(j) < -50.d0) then ! ad hoc modification by MRI
        do i = 1, imf
          if (srft%aexl(i,j,1) == 1.0d0) then ! ocean grid
!            if (icef(i,j) > 0.0d0) then ! on sea ice
!            if (icef(i,j) >= 0.55d0) then ! JRA-55 criteria of sea ice
            if (icjf(i,j) >= 0.5d0) then ! on sea ice (after interpolation)
            ...
            if (alatf(j) < -60.0d0) then ! ice is assumed
              hl1 = (g1 + g2 * satf(i,j)) / (1.0d0 + g3 * satf(i,j)) + i1 * satf(i,j)
              hl2 = 10 ** hl1
              hl3 = 1.0d0 + 1.0d-6 * slpf(i,j)* (h1 + h2 * satf(i,j)**2)
              esat = hl2 * hl3 
    ...
    do j = 1, jmf
      if (alatf(j) < -50.d0) then
        do i = 1, imf
          if (satc(i,j) /= satf(i,j)) then ! if cut-off applied
            if (srft%aexl(i,j,1) == 1.0d0) then ! ocean grid
...
end program calibration_surface_temperature_antarctica
