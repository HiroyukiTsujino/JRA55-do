...
!
subroutine hintpl_mask(datout,imx,jmx,alono,alato,undef_out,&
     &                 datin,imxi,jmxi,aloni,alati,aexl)
  ...
  real(8), intent(in) :: aexl(imxi,jmxi)
  ...
  do j = 1, jmx
    do i = 1, imx
      ...
      hl1 = aexl(iss,jss) + aexl(issp,jss) + aexl(iss,jssp) + aexl(issp,jssp)
      if (hl1+eps > 4.0d0) then
        datout(i,j) = ssic * ssjc * datin(iss,jss)&
             & + ssi  * ssjc * datin(issp,jss )&
             & + ssic * ssj  * datin(iss ,jssp)&
             & + ssi  * ssj  * datin(issp,jssp)
      else
        datout(i,j) = undef_out
      end if

    end do
  end do

end subroutine hintpl_mask
