...
program diagnosis_bulk_flux
  ...
  LOOP_SURF_FORCE: do while ( .true. )
    ...
    do j = 1, jmt1
      do i = 1, imt1
        sst1(i,j) = sst1_file%dat_b(i,j) * sst1_file%br + sst1_file%dat_a(i,j) * sst1_file%ar - tab ! JRA-55 brightness temperature
...
!    end do
    ...
    do j = 1, jmf
      do i = 1, imf
        if (ice1f(i,j) > 0.0d0) then
          sstf(i,j) = aexl(i,j) * sst1f(i,j) ! use brightness temperature over sea ice
        else            
          if (sst2f(i,j) /= undef_out) then
            sstf(i,j) = aexl(i,j) * sst2f(i,j)
          else
    ...
    if (l_bulk_core) then
      call bulk(wsx,wsy,qla,qsn,evp,&
           ...
           &    imf,jmf,aexl,alt_wind,alt_sat,alt_sph)
    else
      ! packing
      do j = 1, jmf
        do i = 1, imf
          ...
          x(i,j,10)= slpf(i,j) !Atmos surface pressure (mb) = (hPa)
      ...
      call bulk_cor30a(imf,jmf,x,y,aexl,jcool,jwave)
