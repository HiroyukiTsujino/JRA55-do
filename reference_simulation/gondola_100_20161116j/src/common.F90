...
contains
  ...
  subroutine setgrd(ftopo, fgrid, fscale, fdz)

    use basin_param

    implicit none

    character(80), intent(in) :: ftopo, fgrid, fscale, fdz
    ...
    open (mtvgrid,file=fdz,form='unformatted')
    read (mtvgrid) k
    write(6,*) ' vgrid(z) ', k
    read (mtvgrid) dz
    close(mtvgrid)

    dz(1:km) = dz(1:km) * 1.d-2
