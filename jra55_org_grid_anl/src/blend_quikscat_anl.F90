...
program blend_quikscat_anl
  ...
  character(256) :: file_u10m_blend_latlon
  character(256) :: file_v10m_blend_latlon
  character(256) :: file_wind_blend_latlon
  ...
  namelist /nml_blend_qscat_anl/ &
       ...
       & file_v10m_blend_org, &
       & file_wind_blend_org, &
       & file_u10m_blend_latlon, &
       & file_v10m_blend_latlon, &
       & file_wind_blend_latlon, &
       & l_out_latlon, &
  ...
  l_out_latlon = .false.
  open(lun,file='namelist.blend_quikscat_anl')
  read(lun,nml=nml_blend_qscat_anl)
  close(lun)
  ...
  do n = 1, total_grid_1d
    if (data_qscat_avail(n) == 1.0d0) then ! needs adjustment
      if (data_wind_org(n) > 0.0d0) then   ! there can be "no wind"
        data_u10m_blend(n) = data_u10m_blend(n) * data_wind_blend(n) / data_wind_org(n)
        data_v10m_blend(n) = data_v10m_blend(n) * data_wind_blend(n) / data_wind_org(n)
      else
  ...
  if (l_out_latlon) then
    ...
    !data_u10m_blend(:) = data_brtmp(:)     ! ad hoc modification for check
    !data_v10m_blend(:) = data_sst_blend(:) ! ad hoc modification for check
    ...
    do j = 1, jmut
      ...
      if (num_xgrid(j) == imut) then
        ...
        u10m_latlon(1:imut,jmut-j+1) = data_u10m_blend(ibgn:iend)
        v10m_latlon(1:imut,jmut-j+1) = data_v10m_blend(ibgn:iend)
        wind_latlon(1:imut,jmut-j+1) = data_wind_blend(ibgn:iend)
        ...
        u10m_org(1:num_xgrid(j)) = data_u10m_blend(ibgn:iend)
        u10m_org(num_xgrid(j)+1) = data_u10m_blend(ibgn)
        v10m_org(1:num_xgrid(j)) = data_v10m_blend(ibgn:iend)
        v10m_org(num_xgrid(j)+1) = data_v10m_blend(ibgn)
        wind_org(1:num_xgrid(j)) = data_wind_blend(ibgn:iend)
        wind_org(num_xgrid(j)+1) = data_wind_blend(ibgn)
    ...
    open(lun,file=file_u10m_blend_latlon,form='unformatted',access='direct',recl=4*imut*jmut)
    write(lun,rec=1) real(u10m_latlon,4)
    close(lun)

    open(lun,file=file_v10m_blend_latlon,form='unformatted',access='direct',recl=4*imut*jmut)
    write(lun,rec=1) real(v10m_latlon,4)
    close(lun)

    open(lun,file=file_wind_blend_latlon,form='unformatted',access='direct',recl=4*imut*jmut)
