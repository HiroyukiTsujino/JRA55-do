! -*-F90-*-
program blend_quikscat_anl_neutral
  ...
  namelist /nml_blend_qscat_anl_neutral/ &
  ...
  l_out_latlon = .false.
  open(lun,file='namelist.blend_quikscat_anl_neutral')
  read(lun,nml=nml_blend_qscat_anl_neutral)
  close(lun)
  ...
  do j = 1, jmut
    ...
    do i = 1, num_xgrid(j)
      ...
      if ((mask_org(i) > 0.0d0) .and. (wind_tmp /= undef8_qscat)) then
  ...
  do j = 1, jmut
    ...
    do i = 1, num_xgrid(j)
      ...
      if (qscat_avail_org(i) == 0.0d0) then
        ...
        if ((weight_total > 0.0d0) .and. (mask_org(i) > 0.0d0)) then
...
end program blend_quikscat_anl_neutral
