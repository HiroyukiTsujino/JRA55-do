...
program diag_actual_wind
  ...
  namelist /nml_jra55_actual_wind/ &
       & file_u10m, file_v10m, &
  ...
  open(lun,file='namelist.jra55_actual_wind')
  read(lun,nml=nml_jra55_actual_wind)
  close(lun)
  ...
  data_un10m(1:total_grid_1d) = real(work4(1:total_grid_1d),8)
  close(lun)

  open(lun,file=file_v10m,form='unformatted',access='direct',convert='little_endian',recl=4*total_grid_1d)
  read(lun,rec=1) work4
  data_vn10m(1:total_grid_1d) = real(work4(1:total_grid_1d),8)
  close(lun)

  do n = 1, total_grid_1d
    data_wvn10m(n) = sqrt(data_un10m(n)**2 + data_vn10m(n)**2)
  end do
  ...
  call wind_neutral_to_real( &
       & data_wind, data_tmp10m, data_sph10m, & ! actual (U,T,Q)
       & data_wvn10m, data_tmp2m, data_sph2m, data_slprs, &
       & data_sst_blend, data_ice, &
       & total_grid_1d, 1, data_mask, &
       & altu, altt, altq, alt_target, sphmin)

  do n = 1, total_grid_1d
    data_u10m(n) = data_un10m(n) * data_wind(n) / data_wvn10m(n)
    data_v10m(n) = data_vn10m(n) * data_wind(n) / data_wvn10m(n)
  end do
