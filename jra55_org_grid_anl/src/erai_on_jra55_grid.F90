...
program erai_on_jra55_grid
  ...
  if (l_flip_mask_erai) then
    do j = 1, jerai
      do i = 1, ierai
        mask_erai(i,j) = 1.d0 - mask_erai(i,j)
      end do
    end do
  end if
      ...
      loop_erai_ii: do ii = 1, ierai + 1
        if ((lon_erai(ii-1) <= model_lon) .and.  (model_lon < lon_erai(ii))) then
          ...
          if (mask_org(i) == 1.0d0) then ! oceanic grid
            if ((mask_erai(iw,js) /= 0.0d0) .and. (mask_erai(ie,js) /= 0.0d0) &
         & .and.(mask_erai(iw,jn) /= 0.0d0) .and. (mask_erai(ie,jn) /= 0.0d0)) then
              sst_tmp = hld1
            else
              weight_total = 0.0d0
              sst_tmp = 0.0d0
              if (mask_erai(iw,js) /= 0.0d0) then
...
end program erai_on_jra55_grid
