! -*-F90-*-
!--------------- shift_wind_real_to_neutral.F90 -----------------------
!   Subroutine bulk
!
...
!
subroutine shift_wind_real2neutral( &
     & wvntrg,tmptrg,sphtrg, &
  ...
  real(8), intent(out) :: wvntrg(imx,jmx)
...
!      if (atexl(i,j) == 1.0d0) then ! needs stability adjustment
        ...
        wv10n = wv ! first guess for 10 m neutral wind
        ...
        LOOP_ADJUST: do n = 1, n_itts                                       ! Monin-Obukhov iteration
          ...
          ! re-evaluation
          !
          wv10n = wv / (1.0d0 + cdn10_rt * (log(altu / 10.0d0) - psi_mu) / karman) ! L-Y eqn. 9a
...
end subroutine shift_wind_real2neutral
