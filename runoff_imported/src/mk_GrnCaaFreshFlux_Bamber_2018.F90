...
program mk_GrnFreshFlux_Bamber
  ...
  character(len=128) :: cfile_in  = 'data_Grn_Bamber_2018_org/grads_monthly_CAA/Bamber_2018_025x025.YYYYMM'
  character(len=128) :: cfile_out_lqd = 'data_Grn_Bamber_2018_new/grads_daily_CAA/Bamber_lqd_2018_025x025.YYYY'
  character(len=128) :: cfile_out_sld = 'data_Grn_Bamber_2018_new/grads_daily_CAA/Bamber_sld_2018_025x025.YYYY'
  ...
  character(len=128) :: cfile_nextxy_new = &
       & 'data_etc/nextxy_wo_lake_big_endian_noyrev_lon0strt_rm_GAC_add_Bamber_2018.bin'  
...
subroutine modify_nextxy
  ...
  open(12,file=cfile_mask,form='unformatted',access='direct',&
       & action='read',recl=4*nx*ny)
  read(12,rec=1) mask_caa
  close(12)
  ...
  do j = 1, ny
    do i = 1, nx
      ...
      if ( nextx_org(i,j) == next_Grn .or. int(mask_caa(i,j)+1.e-4) == 6) then
        nextx_new(i,j) = 0
        nexty_new(i,j) = 0        
      end if
