!-*-F90-*-
program merge_main_grn_caa_ant
  ...
  open(10,file='namelist.merge_main_grn_caa_ant')
  read(10,nml=nml_main_grn_caa_ant)
  close(10)
  ...
  if (l_solid_out) then
    if (.not. l_green_sep) then
      write(6,*) 'WARNING When l_solid_out = .false., l_green_sep = .true. '
      l_green_sep = .true.
    end if
    if (.not. l_antarc_sep) then
      write(6,*) 'WARNING When l_soild_out = .false., l_antarc_sep = .true. '
      l_antarc_sep = .true.
    end if
  end if
  ...
  call open_file_direct(mtinmsk,file_mask_cama,ireclen)
  read(mtinmsk,rec=1) workf(1:nxf,1:nyf)
  mask_cama(1:nxf,1:nyf) = real(workf(1:nxf,1:nyf),8)
  call close_file(mtinmsk)
  ...
  do nyr = ibyr, ieyr
    ...
    do nm = 1, mon_end
      ...
      do nd = 1, day_end
        ...
        do j = 1, nyf
          do i = 1, nxf
            if (int(mask_cama(i,j)+1.d-4) == 6) then 
              rofa(i,j) = 0.d0 ! maskout CAA of CaMaFlood
            end if
          end do
        end do
...
end program merge_main_grn_caa_ant
