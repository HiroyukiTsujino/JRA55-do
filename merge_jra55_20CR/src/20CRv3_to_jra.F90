...
program horizontal_interpolation
  ...
  character(len=256) :: flin1, flin1_base
  character(len=256) :: flot1, flot1_base
  ...
  integer(4) :: nyear
  integer(4) :: nmon, nday, nh, nhour
  integer(4) :: nyearo, nmono, ndayo, nhouro
  integer(4) :: monday
  integer(4) :: idmon(12)
  integer(4) :: idmon_tmp(12)
  logical :: l_leap
  ...
  namelist /nml_hintpol/ flin1_base, flot1_base, dir_out, &
       & undef_out, nyear, l_flux

  !-----------------------------------------------------------------------

  l_flux = .false.
  open (10,file='namelist_20CRv3tojra55')
  ...
  allocate(dat_org(0:imf+1,1:jmf))

  alonf(1:imf) = orgg%lont(1:imf)
  alatf(1:jmf) = orgg%latt(1:jmf)
  ...
  allocate(dat_new(1:imt,1:jmt))

  alond(1:imt) = newg%lonu(1:imt) ! T-point
  alatd(1:jmt) = newg%latu(1:jmt) ! T-point

  lreclen = 4*imt*jmt

  !--------------------------------------------------------------------

  write(flin1,'(1a,1a,i4.4)') trim(flin1_base),'.',nyear
  write(6,*) '  DATA read from ',trim(flin1)
  open(mtin1,file=trim(flin1),form='unformatted',access='direct',recl=4*imf*jmf)
  irec1 = 0
  ...
  idmon_tmp(:) = idmon(:)
  if (l_leap) then
    idmon_tmp(2) = idmon(2) + 1
  end if

  do nmon = 1, 12

    monday = idmon_tmp(nmon)
    ...
    do nday = 1, monday
      ...
      do nh = 1, 8
        ...
        dat_org(1:imf,1:jmf) = real(dat4_org(1:imf,1:jmf),8)
        dat_org(0,1:jmf) = dat_org(imf,1:jmf)
        dat_org(imf+1,1:jmf) = dat_org(1,1:jmf)
        ...
        do j = 1, jmt
          ...
          do i = 1, imt
            ...
            dat_new(i,j) = ssic * ssjc * dat_org(iss ,jss ) &
                 &       + ssi  * ssjc * dat_org(issp,jss ) &
                 &       + ssic * ssj  * dat_org(iss ,jssp) &
                 &       + ssi  * ssj  * dat_org(issp,jssp)
        ...
        if (l_flux) then
          ...
          write(flot1,'(1a,1a,i4.4,i2.2,1a,1a,1a,i4.4,i2.2,i2.2,i2.2)') &
               & trim(dir_out),'/',nyearo, nmono,'/',&
               & trim(flot1_base),'.', nyearo, nmono, ndayo, nhouro
          write(6,*) '  DATA written to ',trim(flot1)
        else
          nhour = 3 * (nh - 1)
          write(flot1,'(1a,1a,i4.4,i2.2,1a,1a,1a,i4.4,i2.2,i2.2,i2.2)') &
               & trim(dir_out),'/',nyear,nmon,'/',&
               & trim(flot1_base),'.', nyear, nmon, nday, nhour
          write(6,*) '  DATA written to ',trim(flot1)
        end if
        ...
        dat4_new(1:imt,1:jmt) = real(dat_new(1:imt,1:jmt),4)
        irec2 = irec2 + 1
