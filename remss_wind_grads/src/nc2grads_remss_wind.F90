...
program read_netcdf_output_grads
  !----------------------------------------------------------------
  ! Information:
  !   Create direct access file (GrADS) of REMSS wind speed data
  !----------------------------------------------------------------
  ...
  real(4),parameter :: undef_land = -999.0
  real(4),parameter :: undef_ice  = -500.0
  ...
  write(flnin(1),'(1a)') '/work116/htsujino/REMSS_WIND/ws_v07r01_198801_201512.nc3.nc'
  ...
  sta(1) = nf_inq_varid(ifiles(1),'wind_speed',var(1))
  ...
  do nyear = 1988, 2015
    ...
    do m = mst, med
      ...
      do j = 1, jmut
        do i = 1, imut
          if ((dat4(i,j) == undef_land) .or. (dat4(i,j) == undef_ice)) then
            swind(i,j) = undef_mxe
          else
            swind(i,j) = dat4(i,j)
          end if
