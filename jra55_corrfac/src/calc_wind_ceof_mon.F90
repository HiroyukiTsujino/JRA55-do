!-*-F90-*-
program calc_wind_ceof_monthly
  ...
  namelist /nml_wind_ceof_mon/ &
       ...
       & slon, elon, slat, elat, &
       & ibyr, ieyr, ibmn, iemn, &
       & l_check, ratio_explain

  !-----------------------------------------------------------------------

  l_check = .false.
  ratio_explain = 0.95d0
  open (11,file='namelist.wind_ceof_mon')
  read (11,nml=nml_wind_ceof_mon)
  close(11)
  ...
  allocate(model_u10m(1:imut,1:jmut), model_v10m(1:imut,1:jmut))
  allocate(nvalid(1:imut,1:jmut))
  ...
  allocate(ene_ratio(imut,jmut))
  allocate(corr_rot_org(imut,jmut))
  ...
  do im = 1, 12
    ...
    write(6,*) 'month = ', im, ' idn = ', idn

    allocate(model_wind(imut,jmut,idn), obs_wind(imut,jmut,idn))

    model_u10m(1:imut,1:jmut) = 0.d0
    model_v10m(1:imut,1:jmut) = 0.d0
    nvalid(1:imut,1:jmut) = 0
      ...
      do j = 1, jmut
        do i = 1, imut
          if ((worku(i,j) /= undef_model) .and. (workv(i,j) /= undef_model)) then
            ...
            model_u10m(i,j) = model_u10m(i,j) + real(worku(i,j),8)
            model_v10m(i,j) = model_v10m(i,j) + real(workv(i,j),8)
    ...
    do j = 1, jmut
      do i = 1, imut
        if (nvalid(i,j) == idn) then
          model_u10m(i,j) = model_u10m(i,j) / real(nvalid(i,j),8)
          model_v10m(i,j) = model_v10m(i,j) / real(nvalid(i,j),8)
        else
          model_u10m(i,j) = 0.0d0
          model_v10m(i,j) = 0.0d0
        end if
      end do
    end do
    ...
    do j = 1, jmut
      do i = 1, imut
        ...
        if (n_valid(i,j) > 0) then
          ...
          ene_ratio(i,j) = abs(w(2)) / (abs(w(1)) + abs(w(2)))

          !if (j==168) then  ! 4N
          !  write(6,'(1a,2f8.2,i4)') ' location, w, ang', grid%glonu(i,j), grid%glatu(i,j), info
          !  write(6,'(3f10.3)') w(1), w(2), corr_rot(i,j)
          !end if
          !
          !if (abs(w(2)) < 19.0d0 * abs(w(1))) then
          ...
          model_vecmag = sqrt(model_u10m(i,j)**2 + model_v10m(i,j)**2)
          !if (ene_ratio(i,j) < ratio_explain) then
          !  corr_mag(i,j) = 1.0d0
          !  corr_rot(i,j) = 0.0d0
          !else
          !  !corr_rot(i,j) = corr_rot(i,j) * 0.5d0 * (1.0d0 + tanh(model_vecmag-3.0d0))  ! smooth transition
          !end if

          corr_rot(i,j) = corr_rot(i,j) * 0.5d0 * (1.0d0 + tanh(1.0d2*(ene_ratio(i,j)-ratio_explain)))  ! smooth transition
          ...
          corr_mag(i,j) = undef_out
          corr_rot(i,j) = undef_out
          corr_rot_org(i,j) = undef_out
          ene_ratio(i,j) = undef_out
        end if
    ...
    write(mtot1,rec=im) real(corr_mag(1:imut,1:jmut),4)
    write(mtot2,rec=3*(im-1)+1) real(corr_rot (1:imut,1:jmut),4)
    write(mtot2,rec=3*(im-1)+2) real(corr_rot_org(1:imut,1:jmut),4)
    write(mtot2,rec=3*(im-1)+3) real(ene_ratio(1:imut,1:jmut),4)

    write(6,*) ' month ', im ,' is done '
...
end program calc_wind_ceof_monthly
