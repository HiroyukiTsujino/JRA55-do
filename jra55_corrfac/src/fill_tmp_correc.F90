...
!==============================================================
  ...
  real(8),allocatable :: tmp_offset(:,:)
  real(8),allocatable :: tmp_offset_filt(:,:)
  real(8),allocatable :: tmp_offset_tmp(:,:)
...
!-----------------------------------------------------------------
  ...
  open (10,file='namelist.fill_tmp_correc')
  read (10,nml_fill_tmp_correc)
  close(10)
...
!-----------------------------------------------------------------
  ...
  allocate(tmp_offset(1:imax,1:jmax))
  allocate(tmp_offset_filt(1:imax,1:jmax))
  allocate(tmp_offset_tmp(0:imax+1,1:jmax))
    ...
    do j = 1, jmax
      do i = 1, imax
        if (mask_land(i,j) == 1.0d0) then
          tmp_offset(i,j) = real(dat2(i,j),8)
        else
          tmp_offset(i,j) = undef8_jra
        end if
      end do
    end do

    tmp_offset_tmp(1:imax,1:jmax) = tmp_offset(1:imax,1:jmax)
    tmp_offset_tmp(0     ,1:jmax) = tmp_offset(  imax,1:jmax)
    tmp_offset_tmp(imax+1,1:jmax) = tmp_offset(     1,1:jmax)
  !------------------------------------------------------------------------
    ...
    do j = 2, jmax - 1
      do i = 1, imax
        if ((tmp_offset_tmp(i,j) == 0.0d0) .or. (tmp_offset_tmp(i-1,j) == 0.0d0) &
             & .or. (tmp_offset_tmp(i+1,j) == 0.0d0) .or. (tmp_offset_tmp(i,j-1) == 0.0d0) &
             & .or. (tmp_offset_tmp(i-1,j-1) == 0.0d0) .or. (tmp_offset_tmp(i+1,j-1) == 0.0d0) &
             & .or. (tmp_offset_tmp(i,j+1) == 0.0d0) .or. (tmp_offset_tmp(i-1,j+1) == 0.0d0) &
             & .or. (tmp_offset_tmp(i+1,j+1) == 0.0d0)) then
    ...
    do i = 1, imax
      if ((tmp_offset_tmp(i,j) == 0.0d0) .or. (tmp_offset_tmp(i-1,j) == 0.0d0) &
           & .or. (tmp_offset_tmp(i+1,j) == 0.0d0) .or. (tmp_offset_tmp(i,j+1) == 0.0d0) &
           & .or. (tmp_offset_tmp(i-1,j+1) == 0.0d0) .or. (tmp_offset_tmp(i+1,j+1) == 0.0d0)) then
        fill_offset(i,j) = 1.0d0 * mask_land(i,j)
      end if
    end do
...
!    fill_offset_tmp(imax+1,1:jmax) = fill_offset(     1,1:jmax)
  ...
  ! 9-point filter
    ...
    tmp_offset_filt(:,:) = tmp_offset(:,:)
  
    do n = 1, num_pass
  
      write(6,'(1a,i2.2)') 'filter pass #', n
  
      tmp_offset_tmp(1:imax,1:jmax) = tmp_offset_filt(1:imax,1:jmax)
      tmp_offset_tmp(0     ,1:jmax) = tmp_offset_filt(  imax,1:jmax)
      tmp_offset_tmp(imax+1,1:jmax) = tmp_offset_filt(     1,1:jmax)
      
      do j = 2, jmax - 1
        do i = 1, imax
          if (fill_offset(i,j) == 1.0d0) then
            hl2 =    tmp_offset_tmp(i  ,j  ) * 12.0d0 * mask_land_tmp(i  ,j  ) &
                 & + tmp_offset_tmp(i+1,j+1) *  1.0d0 * mask_land_tmp(i+1,j+1) &
                 & + tmp_offset_tmp(i  ,j+1) *  2.0d0 * mask_land_tmp(i  ,j+1) &
                 & + tmp_offset_tmp(i-1,j+1) *  1.0d0 * mask_land_tmp(i-1,j+1) &
                 & + tmp_offset_tmp(i+1,j)   *  2.0d0 * mask_land_tmp(i+1,j  ) &
                 & + tmp_offset_tmp(i-1,j)   *  2.0d0 * mask_land_tmp(i-1,j  ) &
                 & + tmp_offset_tmp(i+1,j-1) *  1.0d0 * mask_land_tmp(i+1,j-1) &
                 & + tmp_offset_tmp(i  ,j-1) *  2.0d0 * mask_land_tmp(i  ,j-1) &
                 & + tmp_offset_tmp(i-1,j-1) *  1.0d0 * mask_land_tmp(i-1,j-1)
    ...
    tmp_offset(:,:) = tmp_offset_filt(:,:)
    
  !------------------------------------------------------------------------

    write(mtot1,rec=nd) real(tmp_offset(1:imax,1:jmax),4)
