...
!-----------------------------------------------------------------
  ...
  open (10,file='namelist.windcorrec_mon')
  read (10,nml_windcorrec_mon) 
  close(10)
    ...
    do j = 1, jmax
      do i = 1, imax
        if (mask_wind(i,j) /= 0.0d0) then
          ...
          swind_vec(i,j) = sqrt(uwind_jra(i,j)**2 + vwind_jra(i,j)**2)
        else
    ...
    do j = 1, jmax
      do i = 1, imax
        if (mask_wind(i,j) /= 0.0d0) then
          ...
          swind_vec(i,j) = real(undef_jra,8)
          corr_rot(i,j) = real(undef_jra,8)
        end if
      end do
    end do

    write(mtot2,rec=4*(month-1)+1) real(corr_rot(1:imax,1:jmax),4)
    write(mtot2,rec=4*(month-1)+2) real(swind_vec(1:imax,1:jmax),4)
    write(mtot2,rec=4*(month-1)+3) real(angle_obs(1:imax,1:jmax),4)
    write(mtot2,rec=4*(month-1)+4) real(angle_jra(1:imax,1:jmax),4)
