...
program smooth_correction_factor
  ...
  namelist /nml_smooth_windrot/ flnin, undef_in, flnot, ndata, itermax, &
       & l_apply_mask, fill_value, low_trans, high_trans
  open (11,file='namelist.smooth_windrot')
  read (11,nml=nml_smooth_windrot)
  close(11)
  ...
  do nd = 1, ndata
    ...
    do j = 1, jmut
      ...
      if ((low_trans < abs(grid%latu(j))) .and. (abs(grid%latu(j)) < high_trans)) then
        ...
        do i = 1, imut
          if (mask_wrk(i,j) == 1.0d0) then
            crrc_fin(i,j) = (1.0d0 - hl1) * crrc_fin(i,j) + hl1 * fill_value
          end if
        end do
      end if
