...
!==============================================================
  ...
  real(8) :: tmp10m_obs(imax,jmax)

  real(8) :: tmp10m_jra(imax,jmax)

  real(8) :: mask_jra(imax,jmax)

  real(8) :: tmp10m_crr(imax,jmax)
...
!-----------------------------------------------------------------
  ...
  open (10,file='namelist.tmp10mcorrec')
      ...
      do i = 1, imax
        if (mask_jra(i,j) == 1.0d0) then
          if ((tmp10m_obs(i,j) /= 0.0d0) .and. (tmp10m_jra(i,j) /= 0.0d0)) then
            tmp10m_crr(i,j) = tmp10m_obs(i,j) - tmp10m_jra(i,j)
          else
