!-*-F90-*-
program make_annual_climatology_scaled
  ...
  real(8),allocatable :: data_u_cl3(:)
  real(8),allocatable :: data_v_cl3(:)

  real(8),allocatable :: data_w_cl1(:), data_w_cl2(:)
  ...
  character(256) :: file_u_clim_in_ref
  character(256) :: file_u_clim_in_org
  character(256) :: file_u_clim_out_scl

  character(256) :: file_v_clim_in_ref
  character(256) :: file_v_clim_in_org
  character(256) :: file_v_clim_out_scl

  character(256) :: file_w_clim_in_ref1
  character(256) :: file_w_clim_in_ref2
  ...
  namelist /nml_make_annclim_scaled/ &
       & file_mask,              &
       & undef4_in,              &
       & file_u_clim_in_ref,     &
       & file_u_clim_in_org,     &
  ...
  open(lun,file='namelist.make_annclim_scaled')
  read(lun,nml=nml_make_annclim_scaled)
  close(lun)
  ...
  allocate(data_u_new3(1:imut))
  allocate(data_v_new1(1:imut),data_v_new2(1:imut))
  allocate(data_v_new3(1:imut))
  ...
  allocate(data_w_cl1(1:total_grid_1d))
  allocate(data_w_cl2(1:total_grid_1d))
  ...
  open(lun,file=file_u_clim_in_ref,form='unformatted',access='direct', &
       & convert='little_endian', action='read', recl=4*total_grid_1d)
  read(lun,rec=1) work4
  close(lun)
  data_u_cl1(1:total_grid_1d) = real(work4(1:total_grid_1d),8)

  open(lun,file=file_v_clim_in_ref,form='unformatted',access='direct', &
       & convert='little_endian', action='read', recl=4*total_grid_1d)
  read(lun,rec=1) work4
  close(lun)
  data_v_cl1(1:total_grid_1d) = real(work4(1:total_grid_1d),8)

  open(lun,file=file_w_clim_in_ref1,form='unformatted',access='direct', &
       & convert='little_endian', action='read', recl=4*total_grid_1d)
  read(lun,rec=1) work4
  close(lun)
  data_w_cl1(1:total_grid_1d) = real(work4(1:total_grid_1d),8)

  !----------------------------------------------
  ! ORG 1988-1996

  open(lun,file=file_u_clim_in_org,form='unformatted',access='direct', &
       & convert='little_endian', action='read', recl=4*total_grid_1d)
  read(lun,rec=1) work4
  close(lun)
  data_u_cl2(1:total_grid_1d) = real(work4(1:total_grid_1d),8)

  open(lun,file=file_v_clim_in_org,form='unformatted',access='direct', &
       & convert='little_endian', action='read', recl=4*total_grid_1d)
  read(lun,rec=1) work4
  close(lun)
  ...
  open(lun,file=file_w_clim_in_ref2,form='unformatted',access='direct', &
       & convert='little_endian', action='read', recl=4*total_grid_1d)
  read(lun,rec=1) work4
  close(lun)
  data_w_cl2(1:total_grid_1d) = real(work4(1:total_grid_1d),8)

  !----------------------------------------------

  do n = 1, total_grid_1d
    if (data_mask(n) == 1.0d0) then ! 0 for land, 1 for water
      hl1 = sqrt(data_u_cl1(n)**2 + data_v_cl1(n)**2) / data_w_cl1(n) ! ratio : (mean) / (mean + variability)
      hl2 = sqrt(data_u_cl2(n)**2 + data_v_cl2(n)**2)
      if (hl2 < 3.0d0) then ! tapering for wind speed < 3.0 [m/s]
        weight = (max(hl2, 0.5d0) - 0.5d0) * 0.4d0
        factor = data_w_cl2(n) * hl1 / hl2 * weight + (1.0d0 - weight) ! 1.0 for 0.5 [m/s]
      else
        factor = data_w_cl2(n) * hl1 / hl2
      end if
      data_u_cl3(n) = factor * data_u_cl2(n)
      data_v_cl3(n) = factor * data_v_cl2(n)
    else
      data_u_cl3(n) = data_u_cl2(n)
      data_v_cl3(n) = data_v_cl2(n)
    end if
  end do

  !----------------------------------------------

  write(6,*) ' outfile = ',trim(file_u_clim_out_scl)
  open (lun,file=trim(file_u_clim_out_scl),form='unformatted',access='direct', &
       & convert='little_endian', action='write',recl=4*total_grid_1d)
  write(lun,rec=1) real(data_u_cl3,4)
  close(lun)

  write(6,*) ' outfile = ',trim(file_v_clim_out_scl)
  open (lun,file=trim(file_v_clim_out_scl),form='unformatted',access='direct', &
       & convert='little_endian', action='write',recl=4*total_grid_1d)
  write(lun,rec=1) real(data_v_cl3,4)
  close(lun)
  ...
  IF_LATLON: if (l_out_latlon) then 
    ...
    do j = 1, jmut
      ...
      if (num_xgrid(j) == imut) then
        ...
        deallocate(data_u_org3,data_v_org3)
        deallocate(lon_org)
      end if
      i0 = iend
    end do
...
end program make_annual_climatology_scaled
