...
program blend_iab_npoles_anl
  ...
  real(8) :: wgt_x, wgt_y, tmp_iabp, undef8_iabp, model_lat
  real(4) :: undef4_iabp
  ...
  allocate(tmp2m_iabp(1:iiabp,1:jiabp))
  allocate(work4_2d(1:iiabp,1:jiabp))
  allocate(lon_iabp(1:iiabp),lat_iabp(1:jiabp))
  ...
  do j = 1, jmut
    ...
    if (model_lat >= 50.5d0) then
      ...
      do i = 1, num_xgrid(j)
        loop_iabp_i: do ii = 1, iiabp
          if ( (lon_org(i) < lon_iabp(ii)) ) then
            if (ii > 1) then 
              iw = ii - 1
              ie = ii
              wgt_x = (lon_org(i) - lon_iabp(iw)) / (lon_iabp(ie) - lon_iabp(iw))
            else
              iw = iiabp
              ie = ii
              wgt_x = (lon_org(i) - lon_iabp(iw) + 360.d0) / (lon_iabp(ie) - lon_iabp(iw) + 360.d0)
            end if
