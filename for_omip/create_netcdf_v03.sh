...
do
  ...
  for item in tmp10m sph10m u10m v10m slprs dswrf dlwrf rain snow # adjusted
#  for item in tmp10m sph10m u10m v10m slprs
#  for item in dswrf dlwrf rain snow # adjusted
  do
    sed -e s/@year@/${year}/ \
        -e s/@create@/${creation_date}/ \
        -e s/@num_data_year@/${num_data_year}/ \
        -e s/@num_data@/${num_data}/ \
        ${item}_v03_nc.sh_template > ${item}_nc.sh
