!-*-F90-*-
program make_monthly_time_series
  ...
  namelist /nml_make_monthly_surf/ &
       & iyear, imon,            &
       & int_hour,               &
       & file_in_base,           &
       & var_in,                 &
  ...
  open(lun,file='namelist.make_monthly_surf')
  read(lun,nml=nml_make_monthly_surf)
  close(lun)
  ...
  do nd = 1, nday
    do nh = 1, num_data_per_day
      ...
      open(lun,file=file_surf,form='unformatted',action='read',access='direct', &
           & convert='little_endian',recl=4*total_grid_1d)
  ...
  do n = 1, total_grid_1d
    ...
    if ((weight_all(n) + eps) >= 24.d0 * real(nday,8)) then
      ...
      write(6,*) ' some data are missing, cannot compute mean '
  ...
  write(file_surf_all,'(1a,1a,1a,1a,i4.4,i2.2)') trim(file_out_base),'/',trim(var_out),'.',iyear,imon
  open(lun,file=file_surf_all,form='unformatted',action='write',access='direct', &
       & convert='little_endian',recl=4*total_grid_1d)
  ...
  write(file_latlon_all,'(1a,1a,1a,1a,i4.4,i2.2)') &
       & trim(file_out_latlon_base),'/',trim(var_out),'.',iyear,imon
...
end program make_monthly_time_series
