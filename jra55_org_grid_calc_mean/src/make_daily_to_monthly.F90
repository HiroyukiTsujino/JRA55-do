! -*-F90-*-
!------------------------------------------------------------------------
program make_monthly_data_from_daily
  ...
  namelist /nml_day2mon/ iyst, imst, iyed, imed, &
       & item_name, &
       & in_base_dir, out_base_dir

  !-----------------------------------------------------------------------

  open(lun,file='namelist_daily2monthly')
  read(lun,nml=nml_day2mon)
  close(lun)
  ...
  do m = mst, med
    ...
    do n = 1, idmon(m)
      ...
      do j = 1, jmf
        do i = 1, imf
          if (abs(data_snap(i,j)) > 1.0e10) then
            write(6,*) 'There is missing data. Please check. '
            write(6,*) ' i = ',i, ' j = ',j, data_snap(i,j)
            stop
          end if
        end do
      end do

      data_mon(1:imf,1:jmf) = data_mon(1:imf,1:jmf) + real(data_snap(1:imf,1:jmf),8)
...
end program make_monthly_data_from_daily
