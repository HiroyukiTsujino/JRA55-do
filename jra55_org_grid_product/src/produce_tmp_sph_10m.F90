...
program blend_quikscat_anl
  ...
  real(8),allocatable :: data_tmp2m_org(:)
  ...
  real(8),allocatable :: tmp2m_adj(:), tmp2m_org(:)
  ...
  character(256) :: file_tmp2m_org
  character(256) :: file_ice, file_slprs
  character(256) :: file_mask

  character(256) :: file_tmp10m_org
  character(256) :: file_sph10m_org

  character(256) :: file_tmp10m_latlon
  character(256) :: file_sph10m_latlon
  ...
  open(lun,file='namelist.tmp_sph_10m')
  read(lun,nml=nml_tmp_sph_10m)
  close(lun)
  ...
  allocate(data_tmp2m_org(1:total_grid_1d))
  ...
  open(lun,file=file_tmp2m_org, form='unformatted',access='direct',convert='little_endian',recl=4*total_grid_1d)
  read(lun,rec=1) work4
  data_tmp2m_org(1:total_grid_1d) = real(work4(1:total_grid_1d),8)
  close(lun)
  ...
  do j = 1, jmut
    ...
    allocate(tmp2m_adj(1:num_xgrid(j)))
    ...
    tmp2m_adj(1:num_xgrid(j)) = data_tmp2m(ibgn:iend)
    tmp2m_org(1:num_xgrid(j)) = data_tmp2m_org(ibgn:iend)
    ...
    do i = 1, num_xgrid(j)
      ...
      if ((ice_org(i) == 0.0d0) .and. (mask_org(i) > 0.0d0) .and. (sst_tmp /= undef8_cobe)) then
        ...
        if ((ice_org(i) > 0.0d0)  .and. (mask_org(i) > 0.0d0)) then
          ...
          sst_org(i) = brtmp_org(i) + (tmp2m_adj(i) - tmp2m_org(i))
        else
    ...
    deallocate(tmp2m_org,tmp2m_adj)
    deallocate(lon_org)
  ...
  write(6,*) 'Product written to ', trim(file_tmp10m_org)
  write(6,*) 'Product written to ', trim(file_sph10m_org)

  open(lun,file=file_tmp10m_org,form='unformatted',access='direct',convert='little_endian',recl=4*total_grid_1d)
  write(lun,rec=1) real(data_tmp10m,4)
  close(lun)

  open(lun,file=file_sph10m_org,form='unformatted',access='direct',convert='little_endian',recl=4*total_grid_1d)
  write(lun,rec=1) real(data_sph10m,4)
  close(lun)
  ...
  if (l_out_latlon) then
    ...
    open(lun,file=file_tmp10m_latlon,form='unformatted',access='direct',recl=4*imut*jmut)
    write(lun,rec=1) real(tmp10m_latlon,4)
    close(lun)

    open(lun,file=file_sph10m_latlon,form='unformatted',access='direct',recl=4*imut*jmut)
    write(lun,rec=1) real(sph10m_latlon,4)
    close(lun)

  end if

end program blend_quikscat_anl
