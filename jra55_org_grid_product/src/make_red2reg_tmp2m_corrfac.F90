! -*-F90-*-
program make_reduced_to_regular_tmp2m_corrfac
  ...
  namelist /nml_make_red2reg_tmp2m_corrfac/ &
       & file_in,             &
       & file_latlon,         &
       & file_mask,           &
       & file_mask_latlon,    &
  ...
  open(lun_i,file='namelist.make_red2reg_tmp2m_corrfac')
  read(lun_i,nml=nml_make_red2reg_tmp2m_corrfac)
  close(lun_i)
      ...
      do i = 1, imut
        if (data_land_mask(i,j) > 0.0d0) then
          if (data_weight(i,j) >= 0.5d0) then
            ...
            data_latlon(i,j) = 0.0d0      ! correction factor is missing
          end if
        else
...
end program make_reduced_to_regular_tmp2m_corrfac
