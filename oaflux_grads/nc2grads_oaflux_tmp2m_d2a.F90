...
program read_netcdf_output_grads_d2m
  ...
  do nyear = 1985, 2014
    ...
    write(flnin(1),'(1a,i4.4,1a)') &
         & '/workd/htsujino/OAflux/daily/ta_oaflux_',nyear,'.nc'

    do n = 1, nfiles
      sta(n) = nf_open(flnin(n),nf_nowrite,ifiles(n)) 
      if (sta(n) /= 0) then
        write(6,*) 'nf_open error for file number ',n, flnin(n)
        stop
      end if
    end do

    sta(1) = nf_inq_varid(ifiles(1),'tmp2m',var(1))
        ...
        do j = 1, jmut
          do i = 1, imut
            if (dat4(i,j) /= undef_oaflux) then
              ...
              tmp2m(i,j) = tmp2m(i,j) + real(dat4(i,j),8) * 0.01d0
            end if
          end do
        end do
      end do
    ...
    do j = 1, jmut
      do i = 1, imut
        if (num_valid(i,j) == ndycount) then
          tmp2m(i,j) = tmp2m(i,j) / real(num_valid(i,j))
        else
          tmp2m(i,j) = undef_mxe
        end if
      end do
    ...
    write(flot1,'(1a,i4.4)') '/workd/htsujino/OAflux/grads_annual/tmp2m.',nyear
    open(mtot1,file=flot1,form='unformatted',access='direct',recl=4*imut*jmut)
    write(6,*) 'Temperature written to ',trim(flot1)
    ireco1 = 0
    ireco1 = ireco1 + 1
