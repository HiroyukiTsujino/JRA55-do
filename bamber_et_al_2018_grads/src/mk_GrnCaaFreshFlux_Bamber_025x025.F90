!-*-F90-*-
program mk_GrnCaaFreshFlux_Bamber_025
  implicit none

  character(len=128) :: cfile_in    = 'data_Grn_Bamber_org/grads_monthly/Bamber_2018.YYYYMM'
  character(len=128) :: cfile_out   = 'data_Grn_Bamber_new/grads_monthly_CAA/Bamber_2018_025x025.YYYYMM'
  character(len=128) :: cfile_total = 'data_Grn_Bamber_new/grads_monthly_CAA/total_fwf_Bamber_2018.YYYYMM'
  character(len=128) :: cfile_mask  = 'data_Grn_Bamber_org/mask/mask_Bamber_2018_CAA.gd'
  ...
  open(24,file=cfile_mask,form='unformatted',action='read',access='direct',recl=4*nxi*nyi)
  read(24,rec=1) mask
  close(24)
...
!  write(*,*) trim(cfile)
  ...
  do j = 1, nyi
    do i = 1, nxi
      if ((mask(i,j) == 0.0) .and. (tuni(i,j) /= 0.0)) then
        write(6,*) ' Masking error for tundra data : ', i,j
        stop 999
      end if
      if ((mask(i,j) == 0.0) .and. (mlti(i,j) /= 0.0)) then
        write(6,*) ' Masking error for tundra data : ', i,j
        stop 999
      end if
      if ((mask(i,j) == 0.0) .and. (icei(i,j) /= 0.0)) then
        write(6,*) ' Masking error for tundra data : ', i,j
        stop 999
      end if
      if (int(mask(i,j)+1.e-4) == 1 .or. int(mask(i,j)+1.e-4) == 2) then
        tuni(i,j) = max(tuni(i,j),0.0) * 1.e9     ! (km)^3  -> m^3
        tuni(i,j) = tuni(i,j) / (month_days(nmn) * 86400.e0 ) ! 1/month -> 1/s
        mlti(i,j) = max(mlti(i,j),0.0) * 1.e9     ! (km)^3  -> m^3
        mlti(i,j) = mlti(i,j) / (month_days(nmn) * 86400.e0 ) ! 1/month -> 1/s
        icei(i,j) = max(icei(i,j),0.0) * 1.e9     ! (km)^3  -> m^3
        icei(i,j) = icei(i,j) / (month_days(nmn) * 86400.e0 ) ! 1/month -> 1/s
      else
        tuni(i,j) = 0.d0
        mlti(i,j) = 0.d0
        icei(i,j) = 0.d0
...
end program mk_GrnCaaFreshFlux_Bamber_025
