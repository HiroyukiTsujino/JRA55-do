...
program main
  ...
  integer(4),parameter :: i_search = 5, j_search = 8

  !--------------------------------------------------------------------------

  namelist /nml_remap_on_cobesst/ ibyr, ieyr, riv_in_base, riv_out_base

  !--------------------------------------------------------------------------

  open (10,file='namelist.remap_on_cobesst')
  read (10,nml=nml_remap_on_cobesst)
  close(10)
  ...
  do nyr = ibyr, ieyr
    ...
    do nm = 1, nmonyr
      ...
      open (10,file=file_river_in,form='unformatted',status='old', &
           & access='direct',action='read',recl=4*nx*ny)
      irec = 1
      read(10,rec=irec) work4

      do j = 1, ny
        do i = 1, nx
          rof(i,j) = real(work4(i,j),8) * (newg%areau(i,j) * 1.0d-4)
        end do
      end do

      !-----------------------------------------------
      ! search ocean grids of COBESST
