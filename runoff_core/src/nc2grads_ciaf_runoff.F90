...
!  real(8), parameter :: ro = 1.036d3 ! density of sea water [kg/m3]
  ...
  namelist /nml_core_runoff_ciaf/ fl_in, fl_out_base, file_mask, ibyr, ieyr

  !--------------------------------------------------------------------
 
  open(10, file='namelist.core_runoff_ciaf')
  read(10, nml=nml_core_runoff_ciaf)
  close(10)
  ...
  do nyear = ibyr, ieyr
    ...
    do m = 1, 12
      ...
      do i = 1, 180
        work4(i+180,1:jmut) = rof(i,1:jmut) ! [kg/s/m^2] of pure water
      end do
      do i = 181, 360
        work4(i-180,1:jmut) = rof(i,1:jmut) ! [kg/s/m^2] of pure water
      end do
